...
makeSuite("ParaSwap adapters", (testEnv: TestEnv) => {
  ...
  before(async () => {
    ...
    mockAugustus = await new MockParaSwapAugustus__factory(
  ...
  describe("ParaSwapLiquiditySwapAdapter", () => {
    ...
    describe("executeOperation", () => {
      ...
      it("should correctly swap tokens and deposit the out tokens in the pool", async () => {
        ...
        const adapterWethBalance = await weth.balanceOf(
          paraswapLiquiditySwapAdapter.address,
        );
        const adapterDaiBalance = await dai.balanceOf(
          paraswapLiquiditySwapAdapter.address,
        );
        ...
        expect(userAEthBalance).to.be.gte(
          userAEthBalanceBefore.sub(flashloanTotal),
        );
        expect(userAEthBalance).to.be.lte(
          userAEthBalanceBefore.sub(amountWETHtoSwap),
        );
      });
      ...
      it("should correctly swap tokens using permit", async () => {
        ...
        const daiPrice = await oracle.getAssetPrice(dai.address);
        const expectedDaiAmount = parseEther(
          amountWETHtoSwap.div(daiPrice).toString(),
        );
      ...
      it("should work correctly with tokens of different decimals", async () => {
        ...
        await pool
          .connect(user)
          .deposit(usdc.address, flashloanTotal, userAddress, 0);
        ...
        const expectedDaiAmount = await parseUnitsFromToken(
          ...
          new BigNumber(amountUSDCtoSwap.toString())
            .times(
              new BigNumber(usdcPrice.toString()).times(
                new BigNumber(10).pow(principalDecimals),
              ),
            )
            .div(
              new BigNumber(daiPrice.toString()).times(
        ...
        const aUsdc = await getContract<AToken>(
          eContractid.AToken,
        ...
        const adapterUsdcBalance = await usdc.balanceOf(
          paraswapLiquiditySwapAdapter.address,
        );
        const adapterDaiBalance = await dai.balanceOf(
          paraswapLiquiditySwapAdapter.address,
        ...
        expect(adapterDaiBalance).to.be.eq("0");
        expect(userADaiBalance).to.be.eq(expectedDaiAmount);
        expect(userAUsdcBalance).to.be.gte(
          userAUsdcBalanceBefore.sub(flashloanTotal),
        );
        expect(userAUsdcBalance).to.be.lte(
          userAUsdcBalanceBefore.sub(amountUSDCtoSwap),
        );
      });
      ...
      it("should correctly swap tokens all the balance", async () => {
        ...
        // User will swap liquidity aEth to aDai
        const userAEthBalanceBefore = await aWETH.balanceOf(userAddress);
        expect(userAEthBalanceBefore).to.be.eq(
          amountWETHtoSwap.add(flashloanPremium),
        );
      ...
      it("should not touch any token balance already in the adapter", async () => {
        const { users, weth, oracle, dai, aDai, aWETH, pool, deployer } =
          testEnv;
        const user = users[0].signer;
        const userAddress = users[0].address;
    ...
    describe("executeOperation with borrowing", () => {
      ...
      it("should correctly swap tokens and deposit the out tokens in the pool", async () => {
        ...
        expect(adapterDaiBalance).to.be.eq("0");
        expect(userADaiBalance).to.be.eq(expectedDaiAmount);
        expect(userAEthBalance).to.be.gt(
          userAEthBalanceBefore.sub(flashloanTotal),
        );
    ...
    describe("swapAndDeposit", () => {
      ...
      it("should revert when trying to swap more than balance", async () => {
        ...
        const amountWETHtoSwap = (
          await parseUnitsFromToken(weth.address, "100")
        ).add(1);
...
async function deployParaSwapLiquiditySwapAdapter(
  ...
  return await new ParaSwapLiquiditySwapAdapter__factory(
    await getFirstSigner(),
  ).deploy(poolAddressesProvider, augustusRegistry, owner);
}
