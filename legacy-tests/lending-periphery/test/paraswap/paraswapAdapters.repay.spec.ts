  ...
  describe("ParaswapRepayAdapter", () => {
    ...
    describe("executeOperation", () => {
      it("should correctly swap tokens and repay debt", async () => {
        const { users, pool, weth, aWETH, oracle, dai, helpersContract } =
          testEnv;
        ...
        await pool
          .connect(user)
          .borrow(dai.address, expectedDaiAmount, 1, 0, userAddress);
        ...
        const userDaiStableDebtAmountBefore =
          await daiStableDebtContract.balanceOf(userAddress);
        ...
        await aWETH
          .connect(user)
          .approve(paraswapRepayAdapter.address, flashloanTotal);
      ...
      it("should revert if there is not debt to repay with the specified rate mode", async () => {
        ...
        await weth
          .connect(user)
          ["mint(address,uint256)"](userAddress, amountWETHtoSwap);
        await weth
          .connect(user)
          .transfer(paraswapRepayAdapter.address, amountWETHtoSwap);
        ...
        await pool
          .connect(user)
          .borrow(dai.address, expectedDaiAmount, 2, 0, userAddress);
      ...
      it("should swap, repay debt and pull the needed ATokens leaving no leftovers", async () => {
        ...
        const userDaiStableDebtAmount =
          await daiStableDebtContract.balanceOf(userAddress);
        const userAEthBalance = await aWETH.balanceOf(userAddress);
        const adapterAEthBalance = await aWETH.balanceOf(
          paraswapRepayAdapter.address,
        );
      ...
      it("should correctly swap tokens and repay the whole variable debt", async () => {
        ...
        const userDaiVariableDebtAmountBefore =
          await daiVariableDebtContract.balanceOf(userAddress);
        ...
        const userDaiVariableDebtAmount =
          await daiVariableDebtContract.balanceOf(userAddress);
        const userAEthBalance = await aWETH.balanceOf(userAddress);
        const adapterAEthBalance = await aWETH.balanceOf(
          paraswapRepayAdapter.address,
    ...
    describe("swapAndRepay", () => {
      it("should correctly swap tokens and repay debt", async () => {
        ...
        await aWETH
          .connect(user)
          .approve(paraswapRepayAdapter.address, liquidityToSwap);
        const params = buildParaswapBuyParams(
          mockAugustusCalldata,
          mockAugustus.address,
        );
        ...
        expect(userAEthBalance).to.be.lt(userAEthBalanceBefore);
        expect(userAEthBalance).to.be.gte(
          userAEthBalanceBefore.sub(liquidityToSwap),
        );
      });
      ...
      it("should revert when max amount allowed to swap is bigger than max slippage", async () => {
        ...
        await aWETH
          .connect(user)
          .approve(paraswapRepayAdapter.address, bigMaxAmountToSwap);
      ...
      it("should swap (not whole amount), repay debt and pull the needed ATokens leaving no leftovers", async () => {
        ...
        expect(userAEthBalance).to.be.lt(userAEthBalanceBefore);
        expect(userAEthBalance).to.be.eq(
          userAEthBalanceBefore.sub(swappedAmount),
        );
        expect(userWethBalance).to.be.eq(userWethBalanceBefore);
      });
