...
makeSuite("Faucet", (testEnv: TestEnv) => {
  ...
  describe("Permissioned mode: disabled", () => {
    ...
    it("Mint function should mint tokens within limit", async () => {
      ...
      const threshold = await faucetOwnable
    ...
    it("Non-owner tries to deactivate minting (revert expected)", async () => {
      ...
      await expect(
        faucetOwnable
          .connect(user.signer)
          .setMintable(tokenMintable.address, false),
    ...
    it("Owner deactivates mint", async () => {
      ...
      await expect(
        faucetOwnable
          .connect(user.signer)
          .mint(tokenMintable.address, user.address, 1),
      ...
      await waitForTx(
        await faucetOwnable
          .connect(deployer.signer)
          .setMintable(tokenMintable.address, true),
      );
      expect(await faucetOwnable.isMintable(tokenMintable.address)).to.be.true;
      ...
      expect(await tokenMintable.balanceOf(user.address)).eq(
        balanceBefore.add(1),
      );
    });
  ...
  describe("Permissioned mode: enabled", () => {
    ...
    it("Mint function can only be called by owner", async () => {
      ...
      await expect(await tokenMintable.balanceOf(user.address)).eq(
  ...
  it("Function setMaximumMintAmount should revert if caller not owner", async () => {
    ...
    await expect(
      faucetOwnable.connect(user.signer).setMaximumMintAmount(123),
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });
  ...
  it("Transfer ownership should revert if not owner", async () => {
    ...
    const childContract = await new TestnetERC20__factory(
      deployer.signer,
    ).deploy("CHILD", "CHILD", 18, faucetOwnable.address);
  ...
  it("Direct mint should revert if protected and caller not owner", async () => {
    ...
    await expect(
      tokenMintable
        .connect(user.signer)
        ["mint(address,uint256)"](user.address, 1),
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });
  ...
  it("Direct mint if not protected", async () => {
    ...
    expect(await tokenMintable.isProtected()).to.be.true;
    await expect(
      tokenMintable.connect(user.signer).setProtected(false),
    ).to.be.revertedWith("Ownable: caller is not the owner");
    expect(
      await faucetOwnable.setProtectedOfChild([tokenMintable.address], false),
    );
