...
makeSuite(
  "Use native ETH at Pool via WrappedTokenGatewayV3",
  (testEnv: TestEnv) => {
    ...
    it("Deposit WETH via WrappedTokenGatewayV3 and DAI", async () => {
      ...
      await WrappedTokenGatewayV3.connect(user.signer).depositETH(
        pool.address,
        user.address,
        "0",
        {
    ...
    it("Withdraw WETH - Partial", async () => {
      ...
      const { gasUsed: approveGas }: ContractReceipt =
    ...
    it("Borrow ETH via delegateApprove ETH and repays back", async () => {
      ...
      await waitForTx(
        await WrappedTokenGatewayV3.connect(user.signer).borrowETH(
          pool.address,
          borrowSize,
          "2",
          "0",
    ...
    it("Owner can do emergency token recovery", async () => {
      ...
      await dai
        .connect(user.signer)
        .transfer(WrappedTokenGatewayV3.address, amount);
      ...
      await WrappedTokenGatewayV3.connect(
        deployer.signer,
    ...
    it("Owner can do emergency native ETH recovery", async () => {
      ...
      const selfdestructArtifact = await hre.deployments.deploy(
      ...
      const WrappedTokenGatewayV3AfterRecovery =
        await hre.ethers.provider.getBalance(WrappedTokenGatewayV3.address);
