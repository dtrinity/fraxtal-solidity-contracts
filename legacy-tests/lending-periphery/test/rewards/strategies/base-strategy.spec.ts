...
makeSuite("Base Transfer Strategy", (testEnv: TestEnv) => {
  ...
  it("Emergency withdrawal will allow to transfer tokens from the contract", async () => {
    ...
    await waitForTx(
      await dai["mint(address,uint256)"](
        pullRewardsStrategy.address,
        amountToRecover,
      ),
    );

    const userBalanceBefore = await dai.balanceOf(user1.address);
    const contractBalanceBefore = await dai.balanceOf(
      pullRewardsStrategy.address,
    );
    ...
    const contractBalanceAfter = await dai.balanceOf(
      pullRewardsStrategy.address,
    );

    expect(userBalanceAfter).to.be.equal(
      userBalanceBefore.add(amountToRecover),
    );
    expect(contractBalanceAfter).to.be.equal(
      contractBalanceBefore.sub(amountToRecover),
    );
  });
  ...
  it("Constructor parameters should be saved at deployment", async () => {
    ...
    const artifact = await hre.deployments.deploy(
      "PullRewardsTransferStrategy-Tests",
      {
