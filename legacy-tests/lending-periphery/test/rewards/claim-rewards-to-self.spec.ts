...
makeSuite("AaveIncentivesController claimRewardsToSelf tests", (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      const { rewardsController, stakedAave, aDaiMockV2, stakedTokenStrategy } =
        testEnv;
      ...
      const destinationAddressBalanceBefore =
        await stakedAave.balanceOf(destinationAddress);
      ...
      const unclaimedRewardsStorageBefore =
        await rewardsController.getUserAccruedRewards(userAddress, reward);
      ...
      const unclaimedRewardsStorageAfter =
        await rewardsController.getUserAccruedRewards(userAddress, reward);

      const destinationAddressBalanceAfter =
        await stakedAave.balanceOf(destinationAddress);

      const claimedAmount = destinationAddressBalanceAfter.sub(
        destinationAddressBalanceBefore,
      );
      ...
      let unclaimedRewardsCalc = unclaimedRewardsStorageBefore.add(
        expectedAccruedRewards,
      );
