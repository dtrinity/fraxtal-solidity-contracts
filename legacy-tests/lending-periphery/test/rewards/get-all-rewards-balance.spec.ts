...
const getRewardsBalanceScenarios: ScenarioAction[] = [
  ...
  {
    caseName:
      "Accrued rewards for first reward but second reward emission is 0",
...
makeSuite("AaveIncentivesController getRewardsBalance tests", (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      await aDaiMockV2.handleActionOnAic(
        userAddress,
        totalSupply,
        stakedByUser,
      );
      ...
      const unclaimedRewardsOneBefore =
        await rewardsController.getUserAccruedRewards(userAddress, rewards[0]);

      const unclaimedRewardsTwoBefore =
        await rewardsController.getUserAccruedRewards(userAddress, rewards[1]);

      const allUnclaimedRewardsBefore = unclaimedRewardsOneBefore.add(
        unclaimedRewardsTwoBefore,
      );
      ...
      const expectedAccruedRewards = expectedAccruedRewardsOne.add(
        expectedAccruedRewardsTwo,
      );
      ...
      if (shouldAccrue) {
        ...
        expect(allUnclaimedRewards.toString()).to.be.equal(
          allUnclaimedRewardsBefore.toString(),
        );
      }
    });
  }
