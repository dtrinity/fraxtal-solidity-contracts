 ...
 * @param stateBefore
 * @param stateAfter
 * @param actionBlockTimestamp
 * @param root0
 * @param root0.fieldsEqualToInput
 * @param root0.fieldsEqualToAnother
 * @param root0.fieldsWithCustomLogic
 */
export async function comparatorEngine<
...
>(
  ...
  for (const { fieldName, logic } of fieldsWithCustomLogic) {
    ...
    // @ts-ignore
    expect(
      stateAfter[fieldName],
      `${fieldName} are not correctly updated`,
    ).to.be.closeTo(closeTo, 2);
  }
}
...
export function eventChecker(
  ...
  expect(event.args?.length || 0 / 2).to.be.equal(
    args.length,
    `${name} signature are wrong`,
  );
 ...
 * @param args
 */
export function eventLogChecker(
  log: LogDescription,
  name: string,
  args: any[] = [],
): void {
  expect(log.name).to.be.equal(name, `Incorrect event emitted`);
  expect(log.args?.length || 0 / 2).to.be.equal(
    args.length,
    `${name} signature are wrong`,
  );
  args.forEach((arg, index) => {
