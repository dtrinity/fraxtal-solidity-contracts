import { getBlockTimestamp, waitForTx, ZERO_ADDRESS } from "@aave/deploy-v3";
import { expect } from "chai";

import { makeSuite, TestEnv } from "../helpers/make-suite";

makeSuite(
  "AaveIncentivesController - Claim all rewards on behalf",
  (testEnv: TestEnv) => {
    beforeEach(async () => {
      const { rewardsController, aDaiMockV2, stakedAave, stakedTokenStrategy } =
        testEnv;

      const distributionEndTimestamp =
        (await getBlockTimestamp()) + 1000 * 60 * 60;
    ...
    it("Should claimRewardsOnBehalf revert if called claimer is not authorized", async () => {
      ...
      await waitForTx(
        await aDaiMockV2.setUserBalanceAndSupply("300000", "30000"),
      );
    ...
    it("Should setClaimer pass if called by emission manager", async () => {
      ...
      await expect(
        await rewardsController.getClaimer(userWithRewards.address),
    ...
    it("Should claimRewardsOnBehalf revert if user argument is ZERO_ADDRESS", async () => {
      const { rewardsController, users, aDaiMockV2, deployer, stakedAave } =
        testEnv;
      ...
      await expect(
        rewardsController.setClaimer(ZERO_ADDRESS, thirdClaimer.address),
      )
        .to.emit(rewardsController, "ClaimerSet")
