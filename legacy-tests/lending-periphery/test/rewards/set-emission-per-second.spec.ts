...
makeSuite(
  "AaveIncentivesController V2 setEmissionPerSecond",
  async (testEnv: TestEnv) => {
    it("Configure assets", async () => {
      ...
      // Perform action
      const action =
        await rewardsController.configureAssets(assetConfigsUpdate);
    ...
    it("Tries to update emissions per second not from emission manager", async () => {
      ...
      await expect(
        rewardsController
          ...
          .setEmissionPerSecond(aDaiMockV2.address, [], []),
    ...
    it("EmissionManager updates the emission per second of distributions", async () => {
      ...
      for (let i = 0; i < assetsConfig.length; i++) {
        ...
        await waitForTx(
          await assetsConfig[i].asset.setUserBalanceAndSupply(
            "0",
