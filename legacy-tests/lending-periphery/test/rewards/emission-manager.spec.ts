...
makeSuite("EmissionManager", (testEnv: TestEnv) => {
  ...
  it("Owner sets a new rewards controller", async () => {
    ...
    expect(await emissionManager.getRewardsController()).to.be.eq(
      mockRewardsController.address,
    );
  ...
  it("Non-owner user tries to set a new rewards controller (revert expected)", async () => {
    ...
    await expect(
      emissionManager
        ...
        .setRewardsController(ZERO_ADDRESS),
  ...
  it("Non-owner user tries to set a new emission admin (revert expected)", async () => {
    ...
    expect(
      await emissionManager.getEmissionAdmin(rewardToken.address),
  ...
  it("Non-owner user tries to set an authorized claimer (revert expected)", async () => {
    ...
    await expect(
      emissionManager
        .connect(users[0].signer)
        .setClaimer(users[0].address, users[1].address),
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });
  ...
  it("Owner sets an authorized claimer", async () => {
    ...
    expect(
      await emissionManager.setClaimer(users[0].address, users[1].address),
    );
  });
    ...
    await expect(
      emissionManager
        ...
        .setEmissionPerSecond(
          aDai.address,
          [rewardToken.address, stakedAave.address],
          [200, 300],
        ),
  ...
  it("Test the accessibility of onlyEmissionAdmin modified functions", async () => {
    ...
    const calls = [
      ...
      {
        fn: "setDistributionEnd",
        args: [randomAddress, rewardToken.address, randomNumber],
      },
    ];
    ...
    for (const call of calls) {
      ...
      expect(
        await emissionManager.connect(users[1].signer)[call.fn](...call.args),
      );
    }
  });

  it("Non-emission-admin tries to configure a new reward distribution (revert expected)", async () => {
    const { emissionManager, aDai, aUsdc, rewardToken, stakedAave, users } =
      testEnv;
