...
makeSuite(
  "AaveIncentivesControllerV2 setTransferStrategy",
  (testEnv: TestEnv) => {
    it("Revert at setTransferStrategy if not emissionManager", async () => {
      ...
      await expect(
        rewardsController
          ...
          .setTransferStrategy(ZERO_ADDRESS, ZERO_ADDRESS),
    ...
    it("Update transfer strategy of a incentivized asset", async () => {
      const { rewardsController, deployer, rewardToken, rewardsVault } =
        testEnv;

      const newStrategy = await hre.deployments.deploy(
      ...
      const rewardStrategy = await rewardsController.getTransferStrategy(
        rewardToken.address,
      );
