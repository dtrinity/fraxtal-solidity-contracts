...
makeSuite("AaveIncentivesController getRewardsBalance tests", (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      const assetIndex = await rewardsController.getAssetIndex(
        underlyingAsset,
        reward,
      );
      ...
      expect(assetIndex[1].toString()).to.be.equal(
        expectedAssetIndex.toFixed(),
      );
      ...
      if (shouldAccrue) {
        ...
        expect(unclaimedRewards.toString()).to.be.equal(
          unclaimedRewardsBefore.toString(),
        );
      }
    });
  }
