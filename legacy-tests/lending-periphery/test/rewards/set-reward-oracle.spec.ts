...
makeSuite("AaveIncentivesControllerV2 setRewardOracle", (testEnv: TestEnv) => {
  ...
  it("Revert at setRewardOracle if oracle doesn't have correct interface", async () => {
    ...
    await expect(rewardsController.setRewardOracle(ZERO_ADDRESS, ZERO_ADDRESS))
      .to.be.reverted;
  });
  // TODO: Enabled when impl deploy mock aave oracle
  ...
  //   const newPriceOracle = await deployMockAggregator(
  //     parseEther("600").toString(),
  //   );
  ...
  //   const priceOracle = await rewardsController.getRewardOracle(
  //     stakedAave.address,
  //   );
