...
makeSuite("AaveIncentivesController handleAction tests", (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      await waitForTx(
        await aDaiMockV2.setUserBalanceAndSupply(userBalance, totalSupply),
      );
      ...
      const eventsEmitted =
        handleActionReceipt.events?.map((e) =>
          rewardsController.interface.parseLog(e),
        ) || [];
      const actionBlockTimestamp = await getBlockTimestamp(
        handleActionReceipt.blockNumber,
      );
      ...
      if (!assetDataAfter.index.eq(assetDataBefore.index)) {
        const eventAccrued = eventsEmitted.find(
          ({ name }) => name === "Accrued",
        );
