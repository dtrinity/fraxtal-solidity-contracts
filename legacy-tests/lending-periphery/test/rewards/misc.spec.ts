import { MAX_UINT_AMOUNT, waitForTx, ZERO_ADDRESS } from "@aave/deploy-v3";
...
makeSuite("RewardsController misc tests", (testEnv) => {
  it("Deployment should pass", async () => {
    ...
    await expect(
      (await rewardsController.EMISSION_MANAGER()).toString(),
    ).to.be.equal(peiEmissionManager);
    await expect(
      (await rewardsController.getEmissionManager()).toString(),
    ).to.be.equal(peiEmissionManager);
  });
  ...
  it("Should return same index while multiple asset index updates", async () => {
    ...
    await waitForTx(
      await aDaiMockV2.doubleHandleActionOnAic(users[1].address, "2000", "100"),
    );
  });
  ...
  it("Should claimRewards revert if performTransfer strategy call returns false", async () => {
    ...
    const mockStrategy = await hre.deployments.deploy(
