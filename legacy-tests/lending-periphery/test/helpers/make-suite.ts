...
export async function initializeMakeSuite() {
  ...
  const rewardsController =
    (await getIncentivesV2()) as any as RewardsController;

  const manager = await impersonateAddress(
  ...
  testEnv.aDaiMockV2 = await deployATokenMock(
    rewardsController.address,
    "aDaiV2",
  );
  testEnv.aWethMockV2 = await deployATokenMock(
    rewardsController.address,
    "aWethV2",
  );
  testEnv.aAaveMockV2 = await deployATokenMock(
    rewardsController.address,
    "aAaveV2",
  );
  testEnv.aEursMockV2 = await deployATokenMock(
    rewardsController.address,
    "aEursV2",
    2,
  );
  testEnv.pullRewardsStrategy =
    (await getPullRewardsStrategy()) as PullRewardsTransferStrategy;
  testEnv.stakedTokenStrategy =
    (await getStakedRewardsStrategy()) as any as StakedTokenTransferStrategy;
  testEnv.rewardToken = await getMintableERC20(
  ...
  testEnv.rewardPriceAggregator = (
    await hre.deployments.get(
      `${rewardTokens[0].symbol}${TESTNET_PRICE_AGGR_PREFIX}`,
    )
  ).address;
  ...
  await waitForTx(
    await testEnv.aaveToken
      ...
      ](rewardsVault.address, parseEther("60000000000")),
  );
  await waitForTx(
