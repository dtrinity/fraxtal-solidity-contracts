...
import { DStakeCollateralVault, DStakeRouterDLend, DStakeToken, MockOracleAggregator } from "../../../typechain-types";
import { ERC20_VESTING_NFT_ID, SDUSD_COLLATERAL_VAULT_ID, SDUSD_DSTAKE_TOKEN_ID, SDUSD_ROUTER_ID } from "../../../typescript/deploy-ids";
...
export const createDStakeFixture = deployments.createFixture(async ({ deployments, getNamedAccounts }) => {
  ...
  const collateralVaultDeployment = await deployments.get(SDUSD_COLLATERAL_VAULT_ID);
  ...
  // Get token contracts
  const { contract: dUSD, tokenInfo: dusdInfo } = await getTokenContractForSymbol(dusdDeployer, "dUSD");
  ...
  const setupTestEnvironment = async (): Promise<void> => {
    ...
    if (currentRouter === ZeroAddress) {
      throw new Error("dSTAKE token router not configured - deployment failed");
    }

    if (currentVault === ZeroAddress) {
      throw new Error("dSTAKE token collateral vault not configured - deployment failed");
    }

    if (defaultVaultAsset === ZeroAddress) {
      throw new Error("dSTAKE router defaultDepositVaultAsset not configured - deployment failed");
    }
    ...
    await dUSD.connect(account1Signer).approve(await dStakeToken.getAddress(), parseUnits("10000", DUSD_DECIMALS));
    await dUSD.connect(account2Signer).approve(await dStakeToken.getAddress(), parseUnits("10000", DUSD_DECIMALS));
    await dUSD.connect(deployerSigner).approve(await dStakeToken.getAddress(), parseUnits("100000", DUSD_DECIMALS));
  };
...
export const createDBoostFixture = deployments.createFixture(async ({ deployments, getNamedAccounts }) => {
...
export const createVestingNFTFixture = deployments.createFixture(async ({ deployments, getNamedAccounts }) => {
  ...
  const account1Signer = await hre.ethers.getSigner(dstakeFixture.accounts.testAccount1);
  const account2Signer = await hre.ethers.getSigner(dstakeFixture.accounts.testAccount2);

  // Deposit dUSD to get dSTAKE tokens
  await dstakeFixture.dStakeToken.connect(account1Signer).deposit(depositAmount, dstakeFixture.accounts.testAccount1);
  await dstakeFixture.dStakeToken.connect(account2Signer).deposit(depositAmount, dstakeFixture.accounts.testAccount2);
  ...
  const dstakeBalance1 = await dstakeFixture.dStakeToken.balanceOf(dstakeFixture.accounts.testAccount1);
  const dstakeBalance2 = await dstakeFixture.dStakeToken.balanceOf(dstakeFixture.accounts.testAccount2);

  await dstakeFixture.dStakeToken.connect(account1Signer).approve(vestingNFTAddress, dstakeBalance1);
  await dstakeFixture.dStakeToken.connect(account2Signer).approve(vestingNFTAddress, dstakeBalance2);
