...
export const createVestingNFTFixture = deployments.createFixture(async ({ deployments, getNamedAccounts }): Promise<any> => {
  ...
  try {
    ...
    await dstakeFixture.dStakeToken.connect(account1Signer).deposit(depositAmount, testAccount1);
    await dstakeFixture.dStakeToken.connect(account2Signer).deposit(depositAmount, testAccount2);

    // Get balances
    const dstakeBalance1 = await dstakeFixture.dStakeToken.balanceOf(testAccount1);
    const dstakeBalance2 = await dstakeFixture.dStakeToken.balanceOf(testAccount2);
    ...
    const createVestingPosition = async (user: string, amount: string): Promise<{ tokenId: bigint; receipt: any }> => {
      ...
      const transferEvent = receipt?.logs?.find((log: any) => log.topics[0] === hre.ethers.id("Transfer(address,address,uint256)"));
    ...
    console.log("Vesting NFT deployment may not be available:", (error as Error).message);
