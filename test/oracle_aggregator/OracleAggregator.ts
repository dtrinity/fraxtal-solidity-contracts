...
import { API3Wrapper, MockStaticOracleWrapper, OracleAggregator } from "../../typechain-types";
...
describe("OracleAggregator", () => {
  ...
  beforeEach(async function () {
    ...
    const dexOracleWrapperAddress = (await hre.deployments.get("DexOracleWrapper")).address;

    ({ tokenInfo: fraxInfo } = await getTokenContractForSymbol(dusdDeployer, "FRAX"));
    ({ tokenInfo: sdaiInfo } = await getTokenContractForSymbol(dusdDeployer, "sDAI"));
    ...
    await oracleAggregatorContract.setOracle(fraxInfo.address, dexOracleWrapperAddress);
    await oracleAggregatorContract.setOracle(sdaiInfo.address, dexOracleWrapperAddress);

    // Fetch the MockStaticOracleWrapper contract so we can use it to set prices in tests
    const mockStaticOracleWrapperAddress = (await hre.deployments.get("MockStaticOracleWrapper")).address;
  ...
  describe("Getting asset prices", () => {
    it("should return expected prices for FRAX and sDAI", async function () {
      ...
      const expectedPriceSdai = hre.ethers.parseUnits("1.1", AAVE_ORACLE_USD_DECIMALS);

      const actualPriceFrax = await oracleAggregatorContract.getAssetPrice(fraxInfo.address);
      const actualPriceSdai = await oracleAggregatorContract.getAssetPrice(sdaiInfo.address);
    ...
    it("should handle changing prices", async function () {
      ...
      const newFraxPrice = hre.ethers.parseUnits("1.05", AAVE_ORACLE_USD_DECIMALS);
      await mockStaticOracleWrapperContract.setAssetPrice(fraxInfo.address, newFraxPrice);

      const updatedPriceFrax = await oracleAggregatorContract.getAssetPrice(fraxInfo.address);
    ...
    it("should revert when price is not alive", async function () {
      ...
      await expect(oracleAggregatorContract.getAssetPrice(fraxInfo.address)).to.be.reverted;
    });
  });
  ...
  describe("Managing oracles", () => {
    ...
    it("should revert when setting oracle with wrong decimals", async function () {
      ...
      await expect(oracleAggregatorContract.setOracle(fraxInfo.address, hardPegOracleWrapperAddress))
        .to.be.revertedWithCustomError(oracleAggregatorContract, "UnexpectedBaseUnit")
        .withArgs(fraxInfo.address, hardPegOracleWrapperAddress, BigInt(10) ** BigInt(AAVE_ORACLE_USD_DECIMALS), 1);
    });
    ...
    it("should only allow oracle manager to set oracles", async function () {
      ...
      await expect(
        oracleAggregatorContract.connect(unauthorizedSigner).setOracle(testAccount1, testAccount2),
      ).to.be.revertedWithCustomError(oracleAggregatorContract, "AccessControlUnauthorizedAccount");
    });
  });

  describe("Base currency and units", () => {
    it("should return correct BASE_CURRENCY", async function () {
      expect(await oracleAggregatorContract.BASE_CURRENCY()).to.equal(hre.ethers.ZeroAddress);
    });
    ...
    it("should return correct BASE_CURRENCY_UNIT", async function () {
      ...
      expect(await oracleAggregatorContract.BASE_CURRENCY_UNIT()).to.equal(expectedUnit);
    });
  });

  describe("Price info", () => {
    it("should return correct price info", async function () {
      const [price, isAlive] = await oracleAggregatorContract.getPriceInfo(fraxInfo.address);
...
describe("OracleAggregator using API3", () => {
  ...
  beforeEach(async function () {
    ...
    await oracleAggregatorContract.setOracle(fraxInfo.address, api3WrapperAddress);
  });
  ...
  it("should fetch correct priceInfo and assetPrice for FRAX", async function () {
    ...
    // Test getAssetPrice
    const assetPrice = await oracleAggregatorContract.getAssetPrice(fraxInfo.address);
