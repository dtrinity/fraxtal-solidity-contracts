...
import { API3CompositeWrapperWithThresholding, MockAPI3Oracle } from "../../typechain-types";
...
describe("API3CompositeWrapperWithThresholding", () => {
  ...
  beforeEach(async function () {
    const { mockAPI3OracleFRAXAddress, mockAPI3OracleSFRAXAddress, sfraxToken, api3CompositeWrapperWithThresholdingAddress } =
      await api3OracleFixture();
    sfraxInfo = sfraxToken;

    ({ dusdDeployer } = await getNamedAccounts());

    api3CompositeWrapperWithThresholdingContract = await hre.ethers.getContractAt(
  ...
  describe("Getting asset prices", () => {
    it("should return expected composite price for sFRAX", async function () {
      ...
      const { price: actualPriceSFrax, isAlive: isAliveSFrax } = await api3CompositeWrapperWithThresholdingContract.getPriceInfo(
        sfraxInfo.address,
      );
      ...
      const assetPrice = await api3CompositeWrapperWithThresholdingContract.getAssetPrice(sfraxInfo.address);
    ...
    it("should return fixed price when composite price is above threshold", async function () {
      ...
      await mockAPI3OracleFRAXContract.setMock(api3PriceFrax, await hre.ethers.provider.getBlock("latest").then((b) => b!.timestamp));

      const { price, isAlive } = await api3CompositeWrapperWithThresholdingContract.getPriceInfo(sfraxInfo.address);
    ...
    it("should correctly handle thresholding with 8 decimal precision for both primary and secondary thresholds", async function () {
      ...
      const lowerThreshold1 = hre.ethers.parseUnits("0.99", AAVE_ORACLE_USD_DECIMALS); // $0.99
      const fixedPrice1 = hre.ethers.parseUnits("1.00", AAVE_ORACLE_USD_DECIMALS); // $1.00
      const lowerThreshold2 = hre.ethers.parseUnits("0.98", AAVE_ORACLE_USD_DECIMALS); // $0.98
      const fixedPrice2 = hre.ethers.parseUnits("1.00", AAVE_ORACLE_USD_DECIMALS); // $1.00
      ...
      const feed = await api3CompositeWrapperWithThresholdingContract.compositeFeeds(newAsset);
    ...
    it("should apply thresholds correctly for both primary and secondary prices", async function () {
      ...
      const expectedPrice = (fixedPrice1 * fixedPrice2) / hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS);
      expect(price).to.equal(expectedPrice);
      expect(isAlive).to.be.true;

      // Test that a price below threshold passes through unchanged
      const priceBelowThreshold1 = hre.ethers.parseUnits("0.95", API3_PRICE_DECIMALS); // Below threshold1
      await mockAPI3OracleSFRAXContract.setMock(priceBelowThreshold1, currentBlock.timestamp);

      const { price: priceWithOneBelow } = await api3CompositeWrapperWithThresholdingContract.getPriceInfo(sfraxInfo.address);

      // Now price1 should be unchanged (0.95) while price2 is still fixed at 1.00
      const expectedPriceWithOneBelow =
        (hre.ethers.parseUnits("0.95", AAVE_ORACLE_USD_DECIMALS) * fixedPrice2) / hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS);
    ...
    it("should apply threshold correctly when only price1 has threshold", async function () {
      ...
      await mockAPI3OracleSFRAXContract.setMock(price1Above, currentBlock.timestamp);
      await mockAPI3OracleFRAXContract.setMock(price2, currentBlock.timestamp);

      const { price: priceWithAboveThreshold } = await api3CompositeWrapperWithThresholdingContract.getPriceInfo(sfraxInfo.address);

      // price1 should be fixed at 1.00, price2 should be unchanged at 1.05
      const expectedPriceAbove =
        (fixedPrice1 * hre.ethers.parseUnits("1.05", AAVE_ORACLE_USD_DECIMALS)) / hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS);
      ...
      await mockAPI3OracleSFRAXContract.setMock(price1Below, currentBlock.timestamp);

      const { price: priceWithBelowThreshold } = await api3CompositeWrapperWithThresholdingContract.getPriceInfo(sfraxInfo.address);

      // price1 should be unchanged at 0.95, price2 should be unchanged at 1.05
      const expectedPriceBelow =
        (hre.ethers.parseUnits("0.95", AAVE_ORACLE_USD_DECIMALS) * hre.ethers.parseUnits("1.05", AAVE_ORACLE_USD_DECIMALS)) /
    ...
    it("should revert when getting price for non-existent asset", async function () {
      ...
      await expect(api3CompositeWrapperWithThresholdingContract.getPriceInfo(nonExistentAsset))
        .to.be.revertedWithCustomError(api3CompositeWrapperWithThresholdingContract, "FeedNotSet")
        .withArgs(nonExistentAsset);
      await expect(api3CompositeWrapperWithThresholdingContract.getAssetPrice(nonExistentAsset))
        .to.be.revertedWithCustomError(api3CompositeWrapperWithThresholdingContract, "FeedNotSet")
        .withArgs(nonExistentAsset);
    });
    ...
    it("should return false or revert when price is stale", async function () {
      ...
      const { isAlive } = await api3CompositeWrapperWithThresholdingContract.getPriceInfo(sfraxInfo.address);
      expect(isAlive).to.be.false;

      // getAssetPrice should revert
      await expect(api3CompositeWrapperWithThresholdingContract.getAssetPrice(sfraxInfo.address)).to.be.revertedWithCustomError(
  ...
  describe("Role based access and management", () => {
    it("should allow adding, updating and removing composite feeds", async function () {
      ...
      await expect(api3CompositeWrapperWithThresholdingContract.addCompositeFeed(newAsset, proxy1, proxy2, 0, 0, 0, 0))
        .to.emit(api3CompositeWrapperWithThresholdingContract, "CompositeFeedAdded")
      ...
      await expect(
        ...
        .to.emit(api3CompositeWrapperWithThresholdingContract, "CompositeFeedUpdated")
        .withArgs(
          newAsset,
          hre.ethers.parseUnits("0.99", AAVE_ORACLE_USD_DECIMALS),
          hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS),
      ...
      const updatedFeed = await api3CompositeWrapperWithThresholdingContract.compositeFeeds(newAsset);
      expect(updatedFeed.proxy1).to.equal(proxy1);
      expect(updatedFeed.proxy2).to.equal(proxy2);
      expect(updatedFeed.primaryThreshold.lowerThresholdInBase).to.equal(hre.ethers.parseUnits("0.99", AAVE_ORACLE_USD_DECIMALS));
      expect(updatedFeed.primaryThreshold.fixedPriceInBase).to.equal(hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS));
      ...
      await expect(api3CompositeWrapperWithThresholdingContract.removeCompositeFeed(newAsset))
        .to.emit(api3CompositeWrapperWithThresholdingContract, "CompositeFeedRemoved")
        .withArgs(newAsset);

      const removedFeed = await api3CompositeWrapperWithThresholdingContract.compositeFeeds(newAsset);
    ...
    it("should revert when non-ORACLE_MANAGER tries to add or remove feeds", async function () {
      ...
      await expect(
        api3CompositeWrapperWithThresholdingContract.connect(unauthorizedSigner).addCompositeFeed(newAsset, proxy1, proxy2, 0, 0, 0, 0),
      )
        .to.be.revertedWithCustomError(api3CompositeWrapperWithThresholdingContract, "AccessControlUnauthorizedAccount")
        .withArgs(testAccount1, await api3CompositeWrapperWithThresholdingContract.ORACLE_MANAGER_ROLE());

      await expect(api3CompositeWrapperWithThresholdingContract.connect(unauthorizedSigner).removeCompositeFeed(newAsset))
        .to.be.revertedWithCustomError(api3CompositeWrapperWithThresholdingContract, "AccessControlUnauthorizedAccount")
        .withArgs(testAccount1, await api3CompositeWrapperWithThresholdingContract.ORACLE_MANAGER_ROLE());
    });
  });
