...
import { CurveOracleWrapper, MockCurveStableNGPool } from "../../typechain-types";
...
describe("CurveOracleWrapper", () => {
  ...
  beforeEach(async () => {
    const { curveWrapperAddress, mockPoolAddress, cusdcToken } = await curveOracleFixture();

    ({ dusdDeployer } = await getNamedAccounts());
    cusdcInfo = cusdcToken;

    wrapper = await hre.ethers.getContractAt("CurveOracleWrapper", curveWrapperAddress, await hre.ethers.getSigner(dusdDeployer));
  ...
  describe("Getting asset prices", () => {
    it("should return correct price from pool", async () => {
      const expectedOutput = hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS);
    ...
    it("should revert when price is zero", async () => {
      ...
      const rates = [hre.ethers.parseUnits("0", 18), hre.ethers.parseUnits("0", 18)];
      await mockPool.setStoredRates(rates);
      await mockPool.setPriceOracle(0, 0);

      await expect(wrapper.getAssetPrice(cusdcInfo.address)).to.be.revertedWithCustomError(wrapper, "PriceIsZero");
    });
