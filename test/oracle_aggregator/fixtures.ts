...
export const dexOracleFixture = deployments.createFixture(async ({ deployments }) => {
  ...
  await mockStaticOracleWrapperContract.setAssetPrice(fraxInfo.address, hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS));
  await mockStaticOracleWrapperContract.setAssetPrice(sdaiInfo.address, hre.ethers.parseUnits("1.1", AAVE_ORACLE_USD_DECIMALS));
});

export const api3OracleFixture = deployments.createFixture(async ({ deployments }) => {
  ...
  const { tokenInfo: fraxInfo } = await getTokenContractForSymbol(dusdDeployer, "FRAX");
  const { tokenInfo: sfraxInfo } = await getTokenContractForSymbol(dusdDeployer, "sFRAX");
  ...
  const mockAPI3OracleFRAXContract = await hre.ethers.getContractAt("MockAPI3Oracle", mockAPI3OracleFRAXAddress, deployer);
  await mockAPI3OracleFRAXContract.setMock(hre.ethers.parseUnits("1", API3_PRICE_DECIMALS), currentTimestamp);
  const mockAPI3OracleSFRAXContract = await hre.ethers.getContractAt("MockAPI3Oracle", mockAPI3OracleSFRAXAddress, deployer);
  await mockAPI3OracleSFRAXContract.setMock(hre.ethers.parseUnits("1.1", API3_PRICE_DECIMALS), currentTimestamp);
  ...
  const { address: api3WrapperWithThresholdingAddress } = await deployContract(
    hre,
  ...
  const { address: api3CompositeWrapperWithThresholdingAddress } = await deployContract(
    hre,
  ...
  const api3WrapperContract = await hre.ethers.getContractAt("API3Wrapper", api3WrapperAddress, deployer);
  await api3WrapperContract.setProxy(fraxInfo.address, mockAPI3OracleFRAXAddress);
  await api3WrapperContract.setProxy(sfraxInfo.address, mockAPI3OracleSFRAXAddress);
  ...
  await api3WrapperWithThresholdingContract.setProxy(fraxInfo.address, mockAPI3OracleFRAXAddress);
  await api3WrapperWithThresholdingContract.setProxy(sfraxInfo.address, mockAPI3OracleSFRAXAddress);

  // Add composite feeds to API3CompositeWrapperWithThresholding
  const api3CompositeWrapperWithThresholdingContract = await hre.ethers.getContractAt(
    "API3CompositeWrapperWithThresholding",
...
export const curveOracleFixture = deployments.createFixture(async ({ deployments }) => {
  await deployments.fixture(); // Start from a fresh deployment
  await deployments.fixture(["oracle-aggregator"]);
  ...
  const { tokenInfo: cusdcInfo } = await getTokenContractForSymbol(dusdDeployer, "cUSDC");
  ...
  const mockPool = await hre.ethers.getContractAt("MockCurveStableNGPool", mockPoolAddress, deployer);
  ...
  const mockAPI3OracleUSDCContract = await hre.ethers.getContractAt("MockAPI3Oracle", mockAPI3OracleUSDCAddress, deployer);
  await mockAPI3OracleUSDCContract.setMock(hre.ethers.parseUnits("1", API3_PRICE_DECIMALS), currentTimestamp);

  const { address: curveAPI3CompositeWrapperWithThresholdingAddress } = await deployContract(
    hre,
  ...
  const curveAPI3CompositeWrapperWithThresholdingContract = await hre.ethers.getContractAt(
    "CurveAPI3CompositeWrapperWithThresholding",
    curveAPI3CompositeWrapperWithThresholdingAddress,
    deployer,
  );

  await curveAPI3CompositeWrapperWithThresholdingContract.setAssetConfig(cusdcInfo.address, mockPoolAddress);
