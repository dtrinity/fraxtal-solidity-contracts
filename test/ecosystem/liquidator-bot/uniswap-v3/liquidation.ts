...
import { standardUniswapV3DEXLBPLiquidityFixture, standardUniswapV3DEXLBPLiquidityWithMockOracleFixture } from "../../fixtures";
import { increaseTime } from "../../utils.chain";
import { setMockStaticOracleWrapperPrice, swapExactInputSingleWithApproval } from "../../utils.dex";
import { borrowAsset, depositCollateralWithApproval, liquidateAsset } from "../../utils.lbp";
import { assertBigIntEqualApproximately, assertNumberEqualApproximately } from "../../utils.math";
...
describe("Liquidation scenarios", function () {
  it("can liquidate sFRAX collateral", async function () {
    ...
    const { dexDeployer, testAccount1, testAccount2 } = await getNamedAccounts();
    ...
    const { contract: dusdViaDeployer } = await getTokenContractForSymbol(dexDeployer, "dUSD");
    ...
    assertBigIntEqualApproximately(await dusdContract.balanceOf(testAccount1), ethers.parseUnits("800", dusdInfo.decimals));
    assertBigIntEqualApproximately(await dusdContract.balanceOf(testAccount2), ethers.parseUnits("1000", dusdInfo.decimals));

    console.log("Performing swaps to decrease sFRAX price");

    for (let i = 0; i < 20; i++) {
      await swapExactInputSingleWithApproval(testAccount2, FeeAmount.HIGH, sfraxInfo.address, dusdInfo.address, 1000, 6000);
    ...
    assertBigIntEqualApproximately(await dusdContract.balanceOf(testAccount2), ethers.parseUnits("21390.055864", dusdInfo.decimals));
    assertBigIntEqualApproximately(await sfraxContract.balanceOf(testAccount2), ethers.parseUnits("80000", sfraxInfo.decimals));

    // Liquidate the sFRAX collateral
    await liquidateAsset(sfraxInfo.address, dusdInfo.address, testAccount1, 400, testAccount2);

    assertBigIntEqualApproximately(await dusdContract.balanceOf(testAccount2), ethers.parseUnits("20990.055864", dusdInfo.decimals));
    ...
    assertBigIntEqualApproximately(await dusdContract.balanceOf(testAccount2), ethers.parseUnits("35271.851875", dusdInfo.decimals));
    assertBigIntEqualApproximately(
      await sfraxContract.balanceOf(testAccount2),
    ...
    await liquidateAsset(sfraxInfo.address, dusdInfo.address, testAccount1, 200, testAccount2);

    assertBigIntEqualApproximately(await dusdContract.balanceOf(testAccount2), ethers.parseUnits("35071.851875", dusdInfo.decimals));
    assertBigIntEqualApproximately(
      await sfraxContract.balanceOf(testAccount2),
...
describe("Test getMaxLiquidationAmount()", function () {
  ...
  beforeEach(async function () {
    ...
    ({ contract: sfraxContract, tokenInfo: sfraxInfo } = await getTokenContractForSymbol(testAccount1, "SFRAX"));

    ({ contract: dusdContract, tokenInfo: dusdInfo } = await getTokenContractForSymbol(testAccount1, "dUSD"));

    // First we need some sFRAX to borrow against
    await sfraxViaDeployer.transfer(testAccount1, ethers.parseUnits("1000", sfraxInfo.decimals));
  ...
  it("health factor >= 1, which leads to zero liquidation amount", async function () {
    ...
    const sfraxBalanceAfter = await sfraxContract.balanceOf(testAccount1);
    assertBigIntEqualApproximately(BigInt(sfraxBalanceBefore) - BigInt(sfraxBalanceAfter), ethers.parseUnits("1000", sfraxInfo.decimals));
    assertBigIntEqualApproximately(await dusdContract.balanceOf(testAccount1), ethers.parseUnits("800", dusdInfo.decimals));
    ...
    const { toLiquidateAmount } = await getMaxLiquidationAmount(sfraxInfo, dusdInfo, testAccount1, dexDeployer);
  ...
  it("health factor < 1 which leads to non-zero liquidation amount", async function () {
    ...
    assertNumberEqualApproximately(await getUserHealthFactor(testAccount1), 1.3280558525);
    ...
    assertNumberEqualApproximately(await getUserHealthFactor(testAccount1), 0.903125, 1e-6);

    const { toLiquidateAmount } = await getMaxLiquidationAmount(sfraxInfo, dusdInfo, testAccount1, dexDeployer);
    assertBigIntEqualApproximately(BigInt(toLiquidateAmount.toString()), BigInt("800000000"));
  });
});
