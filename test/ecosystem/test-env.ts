...
import { AMO_MANAGER_ID, COLLATERAL_VAULT_CONTRACT_ID, ISSUER_CONTRACT_ID, REDEEMER_CONTRACT_ID } from "../../utils/deploy-ids";
import { SWAP_ROUTER_ID, UNISWAP_STATIC_ORACLE_WRAPPER_ID } from "../../utils/dex/deploy-ids";
import { POOL_ADDRESSES_PROVIDER_ID, POOL_DATA_PROVIDER_ID } from "../../utils/lending/deploy-ids";
...
export async function loadTestEnv(): Promise<TestEnv> {
  const { dexDeployer, lendingDeployer, dusdDeployer, testAccount1, testAccount2, testAccount3 } = await getNamedAccounts();
  const users = [
  ...
  const addressesProvider = await hre.ethers.getContractAt("PoolAddressesProvider", addressProviderDeployedResult.address, dexSigner);
  const poolAddress = await addressesProvider.getPool();
  const pool = await hre.ethers.getContractAt("Pool", poolAddress, dexSigner);

  const { address: aaveOracleAddress } = await hre.deployments.get(UNISWAP_STATIC_ORACLE_WRAPPER_ID);

  const oracle = await hre.ethers.getContractAt("StaticOracleWrapper", aaveOracleAddress, await hre.ethers.getSigner(lendingDeployer));

  const { contract: dusd } = await getTokenContractForSymbol(dexDeployer, "dUSD");

  const { contract: sfrax } = await getTokenContractForSymbol(dexDeployer, "SFRAX");
  ...
  const helpersContract = await hre.ethers.getContractAt("AaveProtocolDataProvider", dataProvider.address);
  const swapRouter = await hre.deployments.get(SWAP_ROUTER_ID);
  const dusdRedeemerDeployment = await hre.deployments.get(REDEEMER_CONTRACT_ID);
  const dusdRedeemer = await hre.ethers.getContractAt("Redeemer", dusdRedeemerDeployment.address, await hre.ethers.getSigner(dusdDeployer));
  const dusdCollateralVaultDeployment = await hre.deployments.get(COLLATERAL_VAULT_CONTRACT_ID);
  ...
  const dusdIssuer = await hre.ethers.getContractAt("Issuer", dusdIssuerDeployment.address, await hre.ethers.getSigner(dusdDeployer));
