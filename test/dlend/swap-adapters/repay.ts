...
import { DSwapRepayAdapter, DSwapRepayAdapter__factory } from "../../../typechain-types";
...
const setupDSwapAdapter = async (): Promise<DSwapRepayAdapter> => {
  ...
  return await deployDSwapRepayAdapter(await addressesProvider.getAddress(), swapRouter.address, lendingDeployer);
};
...
describe("DSwapRepayAdapter", () => {
  beforeEach(async () => {
    ...
    await depositCollateralWithApproval(userAddress, await fxs.getAddress(), 1000);
  });

  describe("swapAndRepay", () => {
    it("should correctly swap tokens and repay debt", async () => {
      const { dexDeployer, pool, fxs, aFXS, oracle, dusd, swapPoolFee, helpersContract } = await loadTestEnv();
      const user = await hre.ethers.getSigner(dexDeployer);
      const userAddress = user.address;
      const amountFxsToSwap = await parseUnitsFromToken(await fxs.getAddress(), "10");
      ...
      const amountDusdToRepay = await parseUnitsFromToken(
        ...
        new BigNumber(amountFxsToSwap.toString()).times(fxsPrice.toString()).div(dusdPrice.toString()).shiftedBy(-fxsDecimals).toFixed(0),
      );

      // Open user Debt
      await pool.connect(user).borrow(await dusd.getAddress(), amountDusdToRepay, 2, 0, userAddress);
      const dusdVariableDebtTokenAddress = (await helpersContract.getReserveTokensAddresses(await dusd.getAddress()))
        .variableDebtTokenAddress;
      const dusdVariableDebtContract = await hre.ethers.getContractAt("VariableDebtToken", dusdVariableDebtTokenAddress);
      const userDusdVariableDebtAmountBefore = await dusdVariableDebtContract.balanceOf(userAddress);
      const liquidityToSwap = (amountFxsToSwap * 105n) / 100n;

      const userAFxsBalanceBefore = await aFXS.balanceOf(userAddress);
      await aFXS.connect(user).approve(await dswapRepayAdapter.getAddress(), liquidityToSwap);

      const path = solidityPacked(["address", "uint24", "address"], [await dusd.getAddress(), swapPoolFee, await fxs.getAddress()]);

      await dswapRepayAdapter
        .connect(user)
        .swapAndRepay(await fxs.getAddress(), await dusd.getAddress(), liquidityToSwap, amountDusdToRepay, 2, 0, path, {
          amount: 0,
          deadline: 0,
          v: 0,
      ...
      const adapterFxsBalance = await fxs.balanceOf(await dswapRepayAdapter.getAddress());
      const adapterDusdBalance = await dusd.balanceOf(await dswapRepayAdapter.getAddress());
      const userDusdVariableDebtAmount = await dusdVariableDebtContract.balanceOf(userAddress);
      ...
      expect(userAFxsBalance).to.be.gte(userAFxsBalanceBefore - liquidityToSwap);
    });
  });
});
...
async function deployDSwapRepayAdapter(
  ...
  return await new DSwapRepayAdapter__factory(signer).deploy(poolAddressesProvider, routerAddress, owner);
}
