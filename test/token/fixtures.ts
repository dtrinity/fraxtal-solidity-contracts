...
export const standardTestTokenFixture = hre.deployments.createFixture(async ({ deployments }) => {
  ...
  const { tokenInfo: receiptInfo } = await getTokenContractForSymbol(testTokenDeployer, "dUSD");

  const { tokenInfo: collateralInfo } = await getTokenContractForSymbol(testTokenDeployer, "SFRAX");
  ...
  await receiptContract.grantRole(await receiptContract.MINTER_ROLE(), tokenSupplyManagerDeployment.address);
});
