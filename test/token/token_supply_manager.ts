...
import { MintableERC20, TokenSupplyManager, TokenSupplyManagerHarness } from "../../typechain-types";
import { getTokenAmount, getTokenBalance, transferTokenToAccount } from "../ecosystem/utils.token";
...
describe("Test TokenSupplyManager", function () {
  ...
  beforeEach(async function () {
    ...
    // Get named accounts
    ({ testTokenDeployer, testAccount1, dexDeployer: _dexDeployer } = await hre.getNamedAccounts());

    // Initialize contract instances
    _supplyManager = await getTokenSupplyManagerContract(testTokenDeployer);
    supplyManagerHarness = await getTokenSupplyManagerHarnessContract(testTokenDeployer);
  ...
  describe("Decimal conversion", () => {
    it("converts between decimals correctly", async function () {
      ...
      const convertedAmount = await supplyManagerHarness.testConvertAmountBetweenDecimals(3e7, 6, 2);
      assert.equal(convertedAmount.toString(), (3e3).toString());

      // Convert to more decimals
      const convertedAmount2 = await supplyManagerHarness.testConvertAmountBetweenDecimals(1e3, 2, 4);
  ...
  describe("Token issuance and redemption", () => {
    ...
    beforeEach(async function () {
      ...
      await prepareIssueingTest(collateralSymbol, receiptSymbol, depositAmount, testAccount1);
    });

    it("issues and redeems tokens correctly", async function () {
      // Issue the receipt token from the collateral token
      await issueWithApproval(testAccount1, collateralSymbol, receiptSymbol, depositAmount);
      ...
      await redeemWithApproval(testAccount1, collateralSymbol, receiptSymbol, redeemAmount);

      // Make sure the testAccount1 has 30 collateralToken and 70 receiptToken after the redemption
      await assertBalance(testAccount1, collateralSymbol, redeemAmount.toString());
      await assertBalance(testAccount1, receiptSymbol, (depositAmount - redeemAmount).toString());
    });
    ...
    it("fails when redeeming more than balance", async function () {
      ...
      await expect(redeemWithApproval(testAccount1, collateralSymbol, receiptSymbol, 1000)).to.be.rejectedWith(
    ...
    it("fails when issuing with insufficient balance", async function () {
      ...
      await expect(issueWithApproval(testAccount1, collateralSymbol, receiptSymbol, 1000)).to.be.rejectedWith("ERC20InsufficientBalance");
    });
  });
  ...
  describe("Collateral migration", () => {
    ...
    it("prevents non-owner from migrating collateral", async function () {
      ...
      const nonOwnerSupplyManager = await getTokenSupplyManagerContract(testAccount1);
      await expect(nonOwnerSupplyManager.migrateCollateral(testAccount1, 30)).to.be.rejectedWith("OwnableUnauthorizedAccount");
    });

    it("allows owner to migrate collateral", async function () {
      const testTokenDeployerCollateralBalanceBefore = await getTokenBalance(testTokenDeployer, collateralSymbol);

      // Make sure can migrate the collateral token with owner account
      const ownerSupplyManager = await getTokenSupplyManagerContract(testTokenDeployer);
      await ownerSupplyManager.migrateCollateral(testTokenDeployer, await getTokenAmount("30", collateralSymbol));

      const testTokenDeployerCollateralBalanceAfter = await getTokenBalance(testTokenDeployer, collateralSymbol);

      // Make sure the testTokenDeployer increases by 30 collateralToken after the migration
      assert.equal(
        testTokenDeployerCollateralBalanceAfter - testTokenDeployerCollateralBalanceBefore,
...
async function prepareIssueingTest(
  ...
  await transferTokenToAccount(dexDeployer, testAccount, collateralSymbol, initTestAccountAmount);

  // Make sure the testAccount1 has 100 collateralToken after the transfer
  await assertBalance(testAccount, collateralSymbol, initTestAccountAmount.toString());
  await assertBalance(testAccount, receiptSymbol, "0");
}
