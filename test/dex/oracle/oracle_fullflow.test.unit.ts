...
import {
  addLiquidityToDEXPool,
  deployDEX,
  getPoolAddress,
  getPoolFromAddress,
  initDEXPool,
  swapExactInputSimple,
} from "./utils.dex";
...
describe("Testing DEX Oracle as Lending fallback oracle", function () {
  ...
  it("Prepare", async function () {
    ...
    testTokens = await deployTestTokens({
      Token1: [
        getMintInfo(dexDeployer, 1e12),
        getMintInfo(dexLiquidityAdder, 1e12),
      ],
      Token2: [
        getMintInfo(dexDeployer, 1e12),
        getMintInfo(dexLiquidityAdder, 1e12),
      ...
      Token3: [
        getMintInfo(dexDeployer, 1e8),
        getMintInfo(dexLiquidityAdder, 1e8),
      ],
    });
  ...
  it("No pools yet", async function () {
    ...
    // No pools yet
    const res = await oracleContract.getAllPoolsForPair(
      token1Address,
      token2Address,
    );
  ...
  it("Add pools to DEX", async function () {
    ...
    const expectedPoolInfos: {
      ...
      { liquidity: 3129174178988097489815445n },
      { liquidity: 163459499574653485675676n },
    ];
  ...
  it("Can get the price now", async function () {
    ...
    await assertPrice(
      oracleWrapperContract,
      lendingContracts.oracle,
      token2Address,
      50004091n,
      0.50004091,
    );
  ...
  it("If quote token is the base token, the price should be 1 of price unit", async function () {
    ...
    await assertPrice(
      ...
      lendingContracts.oracle,
      token1Address,
      BigInt(10 ** AAVE_ORACLE_USD_DECIMALS),
      1,
    );
  });
  ...
  it("Perform a swap, the price should be changed", async function () {
    ...
    const amountIn = ethers.parseUnits(
      "150000",
      await testTokens.Token1.decimals(),
    );

    await swapExactInputSimple(
      hre,
      dexContracts.router,
      testTokens.Token1,
      testTokens.Token2,
  ...
  it("Perform a swap in a reverted direction, the price should be changed in a reverted direction", async function () {
    ...
    const amountIn = ethers.parseUnits(
      "350000",
      await testTokens.Token2.decimals(),
    );
...
async function assertPrice(
  ...
  chai
    .expect(Number(rawPrice) / 10 ** Number(priceDecimals))
    .to.closeTo(expectedPriceAfterScaling, 0.00001);

  const rawAaveOraclePrice =
    await aaveOracleContract.getAssetPrice(quoteTokenAddress);
...
async function preparePools(
  oracleContract: StaticOracle,
  token1Address: string,
  token2Address: string,
): Promise<void> {
  await oracleContract.prepareAllAvailablePoolsWithTimePeriod(
    token1Address,
    token2Address,
    1,
  );
