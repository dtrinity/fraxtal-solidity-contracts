...
describe("AmoManager", () => {
  ...
  describe("AMO deallocation", () => {
    it("deallocates AMO tokens from an active vault", async function () {
      ...
      await dusdContract.connect(await hre.ethers.getSigner(amoVault)).approve(amoManagerContract.getAddress(), deallocateAmount);
  ...
  describe("AMO supply management", () => {
    it("decreases AMO supply by burning dUSD", async function () {
      ...
      assert.equal(initialDusdSupply - finalDusdSupply, burnAmount, "dUSD total supply should decrease by burn amount");
    });
    ...
    it("calculates total AMO supply correctly", async function () {
      ...
      const expectedTotalSupply = (await dusdContract.balanceOf(amoManagerContract.getAddress())) + allocateAmount;
      const actualTotalSupply = await amoManagerContract.totalAmoSupply();

      assert.equal(actualTotalSupply.toString(), expectedTotalSupply.toString(), "Total AMO supply calculation is incorrect");
    });
  });
  ...
  describe("AMO vault management", () => {
    ...
    it("only admin can enable/disable AMO vaults", async function () {
      ...
      await expect(amoManagerContract.connect(normalUser).enableAmoVault(newVault)).to.be.revertedWithCustomError(
        amoManagerContract,
        "AccessControlUnauthorizedAccount",
      );

      await expect(amoManagerContract.connect(normalUser).disableAmoVault(newVault)).to.be.revertedWithCustomError(
        amoManagerContract,
        "AccessControlUnauthorizedAccount",
      );
    });
  ...
  describe("Access control", () => {
    it("only AMO allocator can allocate AMO", async function () {
      ...
      await expect(amoManagerContract.connect(normalUser).allocateAmo(amoVault, allocateAmount)).to.be.revertedWithCustomError(
        amoManagerContract,
        "AccessControlUnauthorizedAccount",
      );
    });
    ...
    it("only AMO allocator can deallocate AMO", async function () {
      ...
      await expect(amoManagerContract.connect(normalUser).deallocateAmo(amoVault, deallocateAmount)).to.be.revertedWithCustomError(
        amoManagerContract,
        "AccessControlUnauthorizedAccount",
      );
    });
    ...
    it("only AMO allocator can decrease AMO supply", async function () {
      ...
      await expect(amoManagerContract.connect(normalUser).decreaseAmoSupply(burnAmount)).to.be.revertedWithCustomError(
        amoManagerContract,
        "AccessControlUnauthorizedAccount",
      );
    });
