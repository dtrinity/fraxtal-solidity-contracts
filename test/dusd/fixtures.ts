...
export const standardDUSDDEXFixture = deployments.createFixture(
  async ({ deployments }) => {
    await deployments.fixture(); // Start from a fresh deployment to avoid side-effects from other fixtures
    await deployments.fixture(["mock", "dex", "dusd"]); // dUSD ecosystem and dSwap
    const { dexLiquidityAdder, testTokenDeployer, dusdDeployer } =
    ...
    const { contract: sfrax, tokenInfo: sfraxInfo } =
      await getTokenContractForSymbol(testTokenDeployer, "SFRAX");
    ...
    await setupDusdEcosystem(
      hre,
      mockStaticOracleWrapperAddress as string,
      dusdInfo.address,
      dusdDeployer,
    );

    const issuerAddress = (await hre.deployments.get(ISSUER_CONTRACT_ID))
      .address;
    ...
    const initialSFRAXLiquidityAmount =
      initialDUSDLiquidityAmount / sFraxPriceInDusd;
    ...
    await sfrax.mint(
      dexLiquidityAdder,
      hre.ethers.parseUnits(
        initialSFRAXLiquidityAmount.toString(),
        sfraxInfo.decimals,
      ),
    );
...
const mintDUSDWithSFRAXCollateral = async (
  ...
  const dUSDAmount = hre.ethers.parseUnits(
    initialDUSDAmount.toString(),
    dusdInfo.decimals,
  );
  const sFraxAmount = hre.ethers.parseUnits(
  ...
  await issuer
    .connect(await hre.ethers.getSigner(dexLiquidityAdder))
    .issue(sFraxAmount, sfraxInfo.address, dUSDAmount);
...
const setupDusdEcosystem = async (
  ...
  const { address: dusdIssuerAddress } =
