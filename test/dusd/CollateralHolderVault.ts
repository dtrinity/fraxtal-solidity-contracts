...
describe("CollateralHolderVault", () => {
  ...
  beforeEach(async function () {
    ...
    const collateralVaultAddress = (await hre.deployments.get("CollateralHolderVault")).address;
  ...
  describe("Depositing collateral", () => {
    it("successive successful deposits", async function () {
      // Deposit FRAX
      await fraxContract.approve(await collateralVaultContract.getAddress(), hre.ethers.parseUnits("420", fraxInfo.decimals));
      await collateralVaultContract.deposit(hre.ethers.parseUnits("420", fraxInfo.decimals), fraxInfo.address);

      assert.equal(await collateralVaultContract.totalValue(), hre.ethers.parseUnits("420", AAVE_ORACLE_USD_DECIMALS));

      // Deposit USDC
      await usdcContract.approve(await collateralVaultContract.getAddress(), hre.ethers.parseUnits("80", usdcInfo.decimals));
      await collateralVaultContract.deposit(hre.ethers.parseUnits("80", usdcInfo.decimals), usdcInfo.address);

      assert.equal(await collateralVaultContract.totalValue(), hre.ethers.parseUnits("500", AAVE_ORACLE_USD_DECIMALS));

      // Deposit sDAI (assuming it's already set up in the beforeEach)
      const { contract: sdaiContract, tokenInfo: sdaiInfo } = await getTokenContractForSymbol(dusdDeployer, "sDAI");
      await collateralVaultContract.allowCollateral(sdaiInfo.address);

      await sdaiContract.approve(await collateralVaultContract.getAddress(), hre.ethers.parseUnits("100", sdaiInfo.decimals));
      await collateralVaultContract.deposit(hre.ethers.parseUnits("100", sdaiInfo.decimals), sdaiInfo.address);

      assert.equal(await collateralVaultContract.totalValue(), hre.ethers.parseUnits("610", AAVE_ORACLE_USD_DECIMALS));
    });
  });
  ...
  describe("Withdrawing collateral", () => {
    it("only withdrawer can withdraw", async function () {
      ...
      // Withdraw some FRAX
      await collateralVaultContract.withdraw(hre.ethers.parseUnits("351", fraxInfo.decimals), fraxInfo.address);

      assert.equal(await collateralVaultContract.totalValue(), hre.ethers.parseUnits("69", AAVE_ORACLE_USD_DECIMALS));

      // Normal user cannot withdraw
      await expect(
        collateralVaultContract
          .connect(await hre.ethers.getSigner(testAccount1))
          .withdraw(hre.ethers.parseUnits("9", fraxInfo.decimals), fraxInfo.address),
      ).to.be.revertedWithCustomError(collateralVaultContract, "AccessControlUnauthorizedAccount");
    ...
    it("can withdraw to a specific address using withdrawTo", async function () {
      ...
      await collateralVaultContract.withdrawTo(testAccount1, hre.ethers.parseUnits("100", fraxInfo.decimals), fraxInfo.address);

      const finalBalance = await fraxContract.balanceOf(testAccount1);

      assert.equal(finalBalance - initialBalance, hre.ethers.parseUnits("100", fraxInfo.decimals));

      assert.equal(await collateralVaultContract.totalValue(), hre.ethers.parseUnits("320", AAVE_ORACLE_USD_DECIMALS));
    });
  });
  ...
  describe("Exchanging collateral", () => {
    it("exchange exact amount", async function () {
      ...
      await usdcContract.approve(await collateralVaultContract.getAddress(), hre.ethers.parseUnits("100", usdcInfo.decimals));
    ...
    it("exchange max amount", async function () {
      ...
      await usdcContract.approve(await collateralVaultContract.getAddress(), hre.ethers.parseUnits("420", usdcInfo.decimals));
  ...
  describe("Management", () => {
    it("cannot manage collateral assets as a normal user", async function () {
      ...
      await expect(
        collateralVaultContract.connect(await hre.ethers.getSigner(testAccount1)).allowCollateral(fraxInfo.address),
      ).to.be.revertedWithCustomError(collateralVaultContract, "AccessControlUnauthorizedAccount");

      await expect(
        collateralVaultContract.connect(await hre.ethers.getSigner(testAccount1)).disallowCollateral(fraxInfo.address),
      ).to.be.revertedWithCustomError(collateralVaultContract, "AccessControlUnauthorizedAccount");
    });
    ...
    it("cannot set oracle as a normal user", async function () {
      ...
      await expect(
        collateralVaultContract.connect(await hre.ethers.getSigner(testAccount1)).setOracle(testAccount1),
      ).to.be.revertedWithCustomError(collateralVaultContract, "AccessControlUnauthorizedAccount");
    });
  });
  ...
  describe("Error handling", () => {
    it("should revert with UnsupportedCollateral error", async function () {
      const unsupportedCollateral = "0x0000000000000000000000000000000000000001";
      await expect(collateralVaultContract.deposit(hre.ethers.parseUnits("100", 18), unsupportedCollateral))
    ...
    it("should revert with CollateralAlreadyAllowed error", async function () {
      await expect(collateralVaultContract.allowCollateral(fraxInfo.address))
        .to.be.revertedWithCustomError(collateralVaultContract, "CollateralAlreadyAllowed")
    ...
    it("should revert with CollateralNotSupported error", async function () {
      const unsupportedCollateral = "0x0000000000000000000000000000000000000003";
      await expect(collateralVaultContract.disallowCollateral(unsupportedCollateral))
        .to.be.revertedWithCustomError(collateralVaultContract, "CollateralNotSupported")
        .withArgs(unsupportedCollateral);
    });
    ...
    it("should revert with MustSupportAtLeastOneCollateral error", async function () {
      ...
      await expect(collateralVaultContract.disallowCollateral(fraxInfo.address)).to.be.revertedWithCustomError(
        collateralVaultContract,
    ...
    it("should revert with CannotWithdrawMoreValueThanDeposited error", async function () {
      await expect(
        ...
        .to.be.revertedWithCustomError(collateralVaultContract, "CannotWithdrawMoreValueThanDeposited")
        .withArgs(hre.ethers.parseUnits("200", fraxInfo.decimals), hre.ethers.parseUnits("100", usdcInfo.decimals));
    });
    ...
    it("should revert with ToCollateralAmountBelowMin error", async function () {
      await expect(
        ...
        .to.be.revertedWithCustomError(collateralVaultContract, "ToCollateralAmountBelowMin")
        .withArgs(hre.ethers.parseUnits("100", fraxInfo.decimals), hre.ethers.parseUnits("200", fraxInfo.decimals));
    });
  });
});
