...
import { AmoManager, CurveStableSwapNGAmoVault, IERC20, OracleAggregator } from "../../typechain-types";
...
describe("CurveStableSwapNGAmoVault", () => {
  ...
  beforeEach(async function () {
    ...
    // Fetch dependencies
    ({ dusdDeployer, testAccount1, dusdCollateralWithdrawer, dusdRecoverer, dusdAmoTrader } = await getNamedAccounts());
    ...
    const { address: curveAmoVaultAddress } = await hre.deployments.get(CURVE_STABLESWAPNG_AMO_VAULT_ID);
    curveAmoVault = await hre.ethers.getContractAt("CurveStableSwapNGAmoVault", curveAmoVaultAddress);
    ...
    await mockOracleAggregatorContract.setAssetPrice(TOKENS.USDe.address, hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS));
    await mockOracleAggregatorContract.setAssetPrice(TOKENS.USDC.address, hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS));
    await mockOracleAggregatorContract.setAssetPrice(TOKENS.DAI.address, hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS));
    await mockOracleAggregatorContract.setAssetPrice(TOKENS.USDT.address, hre.ethers.parseUnits("1", AAVE_ORACLE_USD_DECIMALS));
    ...
    await oracleAggregator.setOracle(TOKENS.USDe.address, mockOracleAggregatorAddress);
    await oracleAggregator.setOracle(TOKENS.USDC.address, mockOracleAggregatorAddress);
    await oracleAggregator.setOracle(TOKENS.DAI.address, mockOracleAggregatorAddress);
    await oracleAggregator.setOracle(TOKENS.USDT.address, mockOracleAggregatorAddress);
  });

  describe("Constructor", () => {
    it("should set the correct initial values", async () => {
      expect(await curveAmoVault.amoManager()).to.equal(await amoManager.getAddress());
      ...
      // Check roles
      expect(await curveAmoVault.hasRole(await curveAmoVault.DEFAULT_ADMIN_ROLE(), dusdDeployer)).to.be.true;
      expect(await curveAmoVault.hasRole(await curveAmoVault.COLLATERAL_WITHDRAWER_ROLE(), dusdCollateralWithdrawer)).to.be.true;
      expect(await curveAmoVault.hasRole(await curveAmoVault.RECOVERER_ROLE(), dusdRecoverer)).to.be.true;
      expect(await curveAmoVault.hasRole(await curveAmoVault.AMO_TRADER_ROLE(), dusdAmoTrader)).to.be.true;
    });
  });
  ...
  describe("Liquidity Management", () => {
    it("should add liquidity to the Curve pool", async function () {
      ...
      await curveAmoVault.allowCollaterals([TOKENS.USDC.address, TOKENS.USDe.address]);
      ...
      await USDeContract.connect(whale).transfer(await curveAmoVault.getAddress(), USDeAmount);
      await USDCContract.connect(whale).transfer(await curveAmoVault.getAddress(), USDCAmount);

      // Get the pool address
      const poolAddress = POOLS.stableswapng.USDe_USDC.address;

      // The LP token is the same as the pool address
      const lpToken = await ethers.getContractAt("@openzeppelin/contracts-5/token/ERC20/IERC20.sol:IERC20", poolAddress);

      // Snapshot the starting LP token balance
      const lpBalanceBefore = await lpToken.balanceOf(await curveAmoVault.getAddress());
      ...
      await curveAmoVault.connect(await ethers.getSigner(dusdAmoTrader)).addLiquidity(poolAddress, amounts, minLPTokens);

      // Check the LP token balance after adding liquidity
      const lpBalanceAfter = await lpToken.balanceOf(await curveAmoVault.getAddress());
      ...
      const USDCBalanceAfter = await USDCContract.balanceOf(await curveAmoVault.getAddress());
    ...
    it("should add and remove liquidity from the Curve pool", async function () {
      ...
      const USDeAmount = ethers.parseUnits(depositAmount100String, TOKENS.USDe.decimals);
      const USDCAmount = ethers.parseUnits(depositAmount100String, TOKENS.USDC.decimals);
      const amounts = [USDeAmount, USDCAmount];
      ...
      const lpBalance = await lpToken.balanceOf(await curveAmoVault.getAddress());

      // Remove liquidity from the pool
      const minAmounts = [0, 0]; // For simplicity, we're not calculating the minimum amounts here
      await curveAmoVault.connect(await ethers.getSigner(dusdAmoTrader)).removeLiquidity(poolAddress, lpBalance, minAmounts);
      ...
      const USDeBalanceAfterDecimal = Number(ethers.formatUnits(USDeBalanceAfter, TOKENS.USDe.decimals));
      const USDCBalanceAfterDecimal = Number(ethers.formatUnits(USDCBalanceAfter, TOKENS.USDC.decimals));
    ...
    it("should remove liquidity imbalanced from the Curve pool", async function () {
      ...
      const withdrawAmounts = [ethers.parseUnits("80", TOKENS.USDe.decimals), ethers.parseUnits("20", TOKENS.USDC.decimals)];
      await curveAmoVault.connect(await ethers.getSigner(dusdAmoTrader)).removeLiquidityImbalance(poolAddress, withdrawAmounts, lpBalance);
  ...
  describe("Swapping", () => {
    it("should perform a swap using the AMO Vault", async function () {
      ...
      const pools: [AddressLike, AddressLike, AddressLike, AddressLike, AddressLike] = [
        POOLS.stableswapng.USDe_USDC.address,
        ethers.ZeroAddress,
        ethers.ZeroAddress,
        ethers.ZeroAddress,
      ...
      const USDeBalanceBefore = await USDeContract.balanceOf(await curveAmoVault.getAddress());
      ...
      expect(USDCBalanceAfter).to.be.lte(((USDCBalanceBefore + expectedOutput) * BigInt(101)) / BigInt(100));
    });
  });
  ...
  describe("Collateral Management", () => {
    it("should allow new collateral tokens", async () => {
      ...
      // Check initial state
      expect(await curveAmoVault.isCollateralSupported(newCollateral1)).to.be.false;
      expect(await curveAmoVault.isCollateralSupported(newCollateral2)).to.be.false;

      // Allow new collateral tokens
      await curveAmoVault.connect(await ethers.getSigner(dusdDeployer)).allowCollaterals([newCollateral1, newCollateral2]);

      // Check updated state
      expect(await curveAmoVault.isCollateralSupported(newCollateral1)).to.be.true;
      expect(await curveAmoVault.isCollateralSupported(newCollateral2)).to.be.true;
    ...
    it("should disallow collateral tokens", async () => {
      ...
      await curveAmoVault.connect(await ethers.getSigner(dusdDeployer)).allowCollaterals([collateralToRemove]);
      expect(await curveAmoVault.isCollateralSupported(collateralToRemove)).to.be.true;

      // Disallow collateral
      await curveAmoVault.connect(await ethers.getSigner(dusdDeployer)).disallowCollaterals([collateralToRemove]);

      // Check updated state
      expect(await curveAmoVault.isCollateralSupported(collateralToRemove)).to.be.false;
    ...
    it("should correctly report allowed collateral", async () => {
      ...
      await curveAmoVault.connect(await ethers.getSigner(dusdDeployer)).allowCollaterals([allowedCollateral]);

      // Check isCollateralAllowed
      expect(await curveAmoVault.isCollateralSupported(allowedCollateral)).to.be.true;
      expect(await curveAmoVault.isCollateralSupported(notAllowedCollateral)).to.be.false;
    });
    ...
    it("should not allow non-admin to manage collateral", async () => {
      ...
      await expect(
        curveAmoVault.connect(await ethers.getSigner(testAccount1)).allowCollaterals([newCollateral]),
      ).to.be.revertedWithCustomError(curveAmoVault, "AccessControlUnauthorizedAccount");
    });
  });
  ...
  describe("LP Token Management", () => {
    it("should correctly track LP tokens", async function () {
      ...
      const whale2 = await ethers.getImpersonatedSigner(WHALES.binance_pegtokenscollateral);
      const DAI = (await ethers.getContractAt(
        "@openzeppelin/contracts-5/token/ERC20/IERC20.sol:IERC20",
        TOKENS.DAI.address,
      )) as unknown as IERC20;
      await DAI.connect(whale2).transfer(await curveAmoVault.getAddress(), DAIAmount);

      await curveAmoVault.connect(await ethers.getSigner(dusdAmoTrader)).addLiquidity(anotherPoolAddress, anotherAmounts, minLPTokens);
  ...
  describe("Value Calculation", () => {
    it("should correctly calculate total collateral value with only collateral tokens", async function () {
      ...
      await curveAmoVault.connect(await ethers.getSigner(dusdDeployer)).allowCollaterals([TOKENS.USDC.address]);

      // Calculate expected value
      const USDePrice = await oracleAggregator.getAssetPrice(TOKENS.USDe.address);
      const USDCPrice = await oracleAggregator.getAssetPrice(TOKENS.USDC.address);
      const expectedValue =
      ...
      expect(totalValue).to.be.closeTo(expectedValue, ethers.parseUnits("1", 18)); // Allow for small rounding differences
    });
    ...
    it("should correctly calculate total collateral value with multiple LP tokens and collateral", async function () {
      ...
      // Allow tokens as collateral
      await curveAmoVault.allowCollaterals([TOKENS.USDC.address, TOKENS.USDe.address, TOKENS.DAI.address]);
      ...
      await USDeContract.connect(whale1).transfer(await curveAmoVault.getAddress(), USDeAmount1 + USDeAmount2);
      await USDCContract.connect(whale1).transfer(await curveAmoVault.getAddress(), USDCAmount1);
      ...
      const minLPTokens = 0; // For simplicity, we're not calculating the minimum LP tokens here
      await curveAmoVault.connect(await ethers.getSigner(dusdAmoTrader)).addLiquidity(poolAddress1, amounts1, minLPTokens);
      await curveAmoVault.connect(await ethers.getSigner(dusdAmoTrader)).addLiquidity(poolAddress2, amounts2, minLPTokens);
      ...
      // Check if the calculated value matches the expected value
      expect(totalValue).to.be.closeTo(expectedValue, ethers.parseUnits("1", 8)); // Allow for small rounding differences
    });
    ...
    it("should return zero when there are no collateral tokens or LP tokens", async function () {
      ...
      for (const collateral of allCollateral) {
        if (collateral !== (await curveAmoVault.dusd())) {
          await curveAmoVault.connect(await ethers.getSigner(dusdDeployer)).disallowCollaterals([collateral]);
        }
      }
