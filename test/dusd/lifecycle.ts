...
describe("dUSD Ecosystem Lifecycle", () => {
  ...
  beforeEach(async function () {
    ...
    const mockOracleAggregatorAddress = (await hre.deployments.get("MockOracleAggregator")).address;
    ...
    ({ contract: usdcContract, tokenInfo: usdcInfo } = await getTokenContractForSymbol(dusdDeployer, "USDC"));
  ...
  async function calculateWalletValue(wallet: Address): Promise<bigint> {
    const fraxValue = await tokenAmountToUsdValue(await fraxContract.balanceOf(wallet), fraxInfo.address);
    const usdcValue = await tokenAmountToUsdValue(await usdcContract.balanceOf(wallet), usdcInfo.address);
    const dusdValue = await tokenAmountToUsdValue(await dusdContract.balanceOf(wallet), dusdInfo.address);
   ...
   */
  async function convertToEquivalentValueInOutputToken(inputAmount: bigint, inputToken: Address, outputToken: Address): Promise<bigint> {
    const inputPrice = await oracleAggregatorContract.getAssetPrice(inputToken);
    const outputPrice = await oracleAggregatorContract.getAssetPrice(outputToken);
    ...
    const outputAmountInToken = (inputAmountInUsd * 10n ** outputDecimals) / outputPrice;
   ...
   */
  async function tokenAmountToUsdValue(amount: bigint, token: Address): Promise<bigint> {
  ...
  it.skip("two users swap against an AMO vault in a healthy market", async () => {
    ...
    const minInitialDusdForFrax = await convertToEquivalentValueInOutputToken(initialFraxAmount, fraxInfo.address, dusdInfo.address);
    await fraxContract.connect(await hre.ethers.getSigner(testAccount1)).approve(await issuerContract.getAddress(), initialFraxAmount);
    ...
    await amoManagerContract.allocateAmo(await mockAmoVaultContract.getAddress(), vaultAllocation);

    await checkInvariants();

    // 5. User 2 sends 500 USDC to MockAmoVault to simulate swapping on a DEX
    const swapAmount = hre.ethers.parseUnits("500", 18);
    await usdcContract.connect(await hre.ethers.getSigner(testAccount2)).transfer(await mockAmoVaultContract.getAddress(), swapAmount);

    const swapFeeInUsdc = (swapAmount * 1n) / 1000n; // 0.1% fee
    const afterFeeSwapAmount = swapAmount - swapFeeInUsdc;
    const afterFeeSwapAmountInDusd = await convertToEquivalentValueInOutputToken(afterFeeSwapAmount, usdcInfo.address, dusdInfo.address);
    ...
    const transferAmountInUsdc = await convertToEquivalentValueInOutputToken(transferAmountInDusd, dusdInfo.address, usdcInfo.address);
    await dusdContract.connect(await hre.ethers.getSigner(testAccount1)).transfer(testAccount2, transferAmountInDusd);
    await usdcContract.connect(await hre.ethers.getSigner(testAccount2)).transfer(testAccount1, transferAmountInUsdc);
    ...
    await dusdContract
      .connect(await hre.ethers.getSigner(testAccount1))
      .transfer(await mockAmoVaultContract.getAddress(), user1RedeemAmountInDusd);

    const user1RedeemFeesInDusd = (user1RedeemAmountInDusd * 2n) / 100n;
    const user1RedeemAmountInDusdAfterFees = user1RedeemAmountInDusd - user1RedeemFeesInDusd;
    ...
    const user2DusdBalanceBeforeRedeem = await dusdContract.balanceOf(testAccount2);

    await dusdContract
      .connect(await hre.ethers.getSigner(testAccount2))
      .transfer(await mockAmoVaultContract.getAddress(), user2DusdBalanceBeforeRedeem);

    const user2RedeemFeesInDusd = (user2DusdBalanceBeforeRedeem * 2n) / 100n;
    const user2RedeemAmountInDusdAfterFees = user2DusdBalanceBeforeRedeem - user2RedeemFeesInDusd;
    ...
    const availableProfitInUsd = await amoManagerContract.availableProfitInUsd();
    ...
    assert.equal(user1FinalValue, hre.ethers.parseUnits("990", AAVE_ORACLE_USD_DECIMALS), "User 1 should have lost 10 dUSD to fees");
    assert.equal(user2FinalValue, hre.ethers.parseUnits("979.51", AAVE_ORACLE_USD_DECIMALS), "User 2 should have lost 20.49 dUSD to fees");

    const user1LossValue = user1InitialValue - user1FinalValue;
    const user2LossValue = user2InitialValue - user2FinalValue;

    const swapFeeValue = await tokenAmountToUsdValue(swapFeeInUsdc, usdcInfo.address);
    const user1RedeemFeesValue = await tokenAmountToUsdValue(user1RedeemFeesInDusd, dusdInfo.address);
    const user2RedeemFeesValue = await tokenAmountToUsdValue(user2RedeemFeesInDusd, dusdInfo.address);
    const totalFees = swapFeeValue + user1RedeemFeesValue + user2RedeemFeesValue;

    assert.equal(user1LossValue + user2LossValue, totalFees, "Total fees should equal the sum of User 1 and User 2 losses");
    assert.equal(availableProfitInUsd, totalFees, "Available profit in USD should equal total fees collected");
    ...
    assert.equal(endingDusdCirculatingSupply, hre.ethers.parseUnits("0", 6), "We should have redeemed all circulating dUSD");
    assert.equal(endingDusdSupply, endingDusdAmoSupply, "The only remaining dUSD should be the AMO dUSD");
  });
  ...
  it.skip("two users swap in a USDC depeg market", async () => {
    ...
    const minDusdForFrax = await convertToEquivalentValueInOutputToken(depositAmount, fraxInfo.address, dusdInfo.address);
    await fraxContract.connect(await hre.ethers.getSigner(testAccount1)).approve(await issuerContract.getAddress(), depositAmount);
    await issuerContract.connect(await hre.ethers.getSigner(testAccount1)).issue(depositAmount, fraxInfo.address, minDusdForFrax);

    await checkInvariants();

    // 3. User 2 mints 500 dUSD with USDC
    const minDusdForUsdc = await convertToEquivalentValueInOutputToken(depositAmount, usdcInfo.address, dusdInfo.address);
    await usdcContract.connect(await hre.ethers.getSigner(testAccount2)).approve(await issuerContract.getAddress(), depositAmount);
    await issuerContract.connect(await hre.ethers.getSigner(testAccount2)).issue(depositAmount, usdcInfo.address, minDusdForUsdc);

    await checkInvariants();

    // 4. USDC depegs to $0.90
    const depegPrice = hre.ethers.parseUnits("0.90", AAVE_ORACLE_USD_DECIMALS);
    await mockOracleAggregatorContract.setAssetPrice(usdcInfo.address, depegPrice);

    // We are now undercollateralized by $50
    const circulatingSupplyAt4 = await issuerContract.circulatingDusd();
    const totalCollateralValueInDusdAt4 = await issuerContract.collateralInDusd();
    const underCollateralizedAmountInDusdAt4 = circulatingSupplyAt4 - totalCollateralValueInDusdAt4;
    assert.equal(
    ...
    const depeggedMinDusdForUsdc = await convertToEquivalentValueInOutputToken(depositAmount, usdcInfo.address, dusdInfo.address);
    await usdcContract.connect(await hre.ethers.getSigner(testAccount2)).approve(await issuerContract.getAddress(), depositAmount);
    await issuerContract.connect(await hre.ethers.getSigner(testAccount2)).issue(depositAmount, usdcInfo.address, depeggedMinDusdForUsdc);
    ...
    await dusdContract.connect(await hre.ethers.getSigner(testAccount2)).transfer(testAccount1, swapDusdAmount);
    await fraxContract.connect(await hre.ethers.getSigner(testAccount1)).transfer(testAccount2, swapFraxAmount);

    // 7. USDC repegs to $1.00
    const repegPrice = hre.ethers.parseUnits("1.00", AAVE_ORACLE_USD_DECIMALS);
    await mockOracleAggregatorContract.setAssetPrice(usdcInfo.address, repegPrice);
    ...
    const totalCollateralValueInDusdAt7 = await issuerContract.collateralInDusd();
    const overCollateralizedAmountInDusdAt7 = totalCollateralValueInDusdAt7 - circulatingSupplyAt7;
    assert.equal(
    ...
    assert.equal(user1Profit, hre.ethers.parseUnits("50", AAVE_ORACLE_USD_DECIMALS), "User 1 should have profited by $50");
    ...
    assert.equal(user2Loss, hre.ethers.parseUnits("-100", AAVE_ORACLE_USD_DECIMALS), "User 2 should have lost $100");
    ...
    assert.equal(circulatingSupply, hre.ethers.parseUnits("1450", 6), "System should have minted 1450 dUSD in total");
  });
});
