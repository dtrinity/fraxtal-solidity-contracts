...
import { AmoManager, CollateralHolderVault, Issuer, MintableERC20, RedeemerV2 } from "../../typechain-types";
...
describe("RedeemerV2", () => {
  ...
  beforeEach(async function () {
    ...
    const redeemerV2Deployment = await hre.deployments.deploy("RedeemerV2", {
      from: dusdDeployer,
      args: [collateralVaultAddress, dusdInfo.address, oracleAddress, feeReceiver, initialRedemptionFeeBps],
      contract: "RedeemerV2",
      autoMine: true,
    });
    ...
    await collateralVaultContract.grantRole(await collateralVaultContract.COLLATERAL_WITHDRAWER_ROLE(), redeemerV2Deployment.address);
    ...
    await fraxContract.connect(await hre.ethers.getSigner(testAccount1)).approve(await issuerContract.getAddress(), fraxAmount);

    await issuerContract.connect(await hre.ethers.getSigner(testAccount1)).issue(fraxAmount, fraxInfo.address, minDUSD);
  });
  ...
  describe("Permissionless redemption with fees", () => {
    it("should redeem dUSD for collateral with fees", async function () {
      ...
      await dusdContract.connect(await hre.ethers.getSigner(testAccount1)).approve(await redeemerV2Contract.getAddress(), dusdAmount);

      await redeemerV2Contract.connect(await hre.ethers.getSigner(testAccount1)).redeem(dusdAmount, fraxInfo.address, minNetCollateral);
    ...
    it("should revert if slippage too high", async function () {
      ...
      await expect(
        redeemerV2Contract.connect(await hre.ethers.getSigner(testAccount1)).redeem(dusdAmount, fraxInfo.address, minNetCollateral),
    ...
    it("should revert when asset redemption is paused", async function () {
      ...
      await expect(
        redeemerV2Contract.connect(await hre.ethers.getSigner(testAccount1)).redeem(dusdAmount, fraxInfo.address, minNetCollateral),
      ).to.be.revertedWithCustomError(redeemerV2Contract, "AssetRedemptionPaused");
    });
  });
  ...
  describe("Protocol redemption (fee-less)", () => {
    it("should allow protocol redemption without fees", async function () {
      ...
      await dusdContract.approve(await redeemerV2Contract.getAddress(), dusdAmount);

      await redeemerV2Contract.redeemAsProtocol(dusdAmount, fraxInfo.address, minCollateral);
  ...
  describe("Asset-level redemption controls", () => {
    it("should allow setting asset redemption pause", async function () {
      expect(await redeemerV2Contract.isAssetRedemptionEnabled(fraxInfo.address)).to.be.true;

      await redeemerV2Contract.setAssetRedemptionPause(fraxInfo.address, true);
      expect(await redeemerV2Contract.isAssetRedemptionEnabled(fraxInfo.address)).to.be.false;
      expect(await redeemerV2Contract.assetRedemptionPaused(fraxInfo.address)).to.be.true;

      await redeemerV2Contract.setAssetRedemptionPause(fraxInfo.address, false);
      expect(await redeemerV2Contract.isAssetRedemptionEnabled(fraxInfo.address)).to.be.true;
      expect(await redeemerV2Contract.assetRedemptionPaused(fraxInfo.address)).to.be.false;
    });
    ...
    it("should revert when setting pause for unsupported collateral", async function () {
      ...
      await expect(redeemerV2Contract.setAssetRedemptionPause(randomAddress, true)).to.be.revertedWithCustomError(
  ...
  describe("Fee management", () => {
    it("should allow updating default redemption fee", async function () {
      ...
      expect(await redeemerV2Contract.defaultRedemptionFeeBps()).to.equal(newFeeBps);
    });

    it("should allow setting per-collateral redemption fees", async function () {
      const newFeeBps = 50; // 0.5%

      await redeemerV2Contract.setCollateralRedemptionFee(fraxInfo.address, newFeeBps);
      expect(await redeemerV2Contract.collateralRedemptionFeeBps(fraxInfo.address)).to.equal(newFeeBps);
      expect(await redeemerV2Contract.isCollateralFeeOverridden(fraxInfo.address)).to.be.true;
    });
    ...
    it("should allow clearing per-collateral fee overrides", async function () {
      ...
      expect(await redeemerV2Contract.isCollateralFeeOverridden(fraxInfo.address)).to.be.false;
      expect(await redeemerV2Contract.collateralRedemptionFeeBps(fraxInfo.address)).to.equal(0);
    });
    ...
    it("should revert when setting fee too high", async function () {
      ...
      await expect(redeemerV2Contract.setDefaultRedemptionFee(tooHighFeeBps)).to.be.revertedWithCustomError(
        redeemerV2Contract,
        "FeeTooHigh",
      );
    });
  ...
  describe("View functions", () => {
    it("should correctly convert dUSD amount to base value", async function () {
      const dusdAmount = hre.ethers.parseUnits("1000", dusdInfo.decimals);
      const baseValue = await redeemerV2Contract.dusdAmountToBaseValue(dusdAmount);
