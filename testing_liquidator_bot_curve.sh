...
# Migration file path
MIGRATION_FILE="deployments/fraxtal_testnet/.migrations.json"
...
if [ "$REDEPLOY" = "true" ]; then
    ...
    # Redeploy contracts
    make deploy-contract.liquidator-bot.fraxtal_testnet
fi

# Run liquidation script
echo "Running liquidation script..."
yarn hardhat run --network fraxtal_testnet scripts/liquidator-bot/curve/liquidate_specific_users.ts
