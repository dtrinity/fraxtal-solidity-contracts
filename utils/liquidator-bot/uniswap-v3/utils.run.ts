...
/**
 * Get all users in the Lending Pool for UniswapV3 liquidation
 *
...
export async function getAllLendingUserAddresses(): Promise<string[]> {
  ...
  const graphUrl = config.liquidatorBotUniswapV3.graphConfig.url;

  if (graphUrl.length < 10) {
    throw Error("Invalid graph URL: " + graphUrl);
  }

  const batchSize = config.liquidatorBotUniswapV3.graphConfig.batchSize;
...
/**
 * Get the user liquidation parameters for UniswapV3
 *
...
export async function getUserLiquidationParams(userAddress: string): Promise<{
  ...
  const reserveInfos: UserReserveInfo[] = await batchProcessing(
    reserveAddresses,
    config.liquidatorBotUniswapV3.reserveBatchSize,
