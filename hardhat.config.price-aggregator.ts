...
/**
 * We separate the configuration for the test price aggregator deployment from the default
 * configuration to avoid conflicts re-deploying the price aggregator when running the `make deploy-contract.<network>.reset`
...
const config: HardhatUserConfig = {
  ...
  paths: {
    ...
    cache: "./cache",
    artifacts: "./artifacts",
    deployments: "./deployments/test-tokens", // Share the same deployment folder with test tokens to allow loading the token deployment addresses
  },
