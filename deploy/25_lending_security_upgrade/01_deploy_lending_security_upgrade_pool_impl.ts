...
import {
  SECURITY_UPGRADE_FLASH_LOAN_LOGIC_ID,
  SECURITY_UPGRADE_L2_POOL_IMPL_ID,
} from "../../utils/lending/security-upgrade-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment): Promise<boolean> {
  ...
  const flashLoanLogicDeployment = await hre.deployments.deploy(
    SECURITY_UPGRADE_FLASH_LOAN_LOGIC_ID,
    {
  ...
  const poolImplementationDeployment = await hre.deployments.deploy(
    SECURITY_UPGRADE_L2_POOL_IMPL_ID,
    {
      from: deployer.address,
      contract: "L2Pool",
  ...
  const poolImplementation = await hre.ethers.getContractAt(
    "L2Pool",
    poolImplementationDeployment.address,
    deployer,
  );
  ...
  const currentImplementationStorage = await hre.ethers.provider.send(
    "eth_getStorageAt",
    [poolProxyAddress, IMPLEMENTATION_SLOT, "latest"],
  );
  const currentImplementationAddress = storageToAddress(
    currentImplementationStorage,
  ).toLowerCase();

  if (
    currentImplementationAddress ===
    poolImplementationDeployment.address.toLowerCase()
  ) {
    console.log(
      `Fraxtal lending security upgrade: proxy already points to ${poolImplementationDeployment.address}.`,
    );
    return true;
  }
  ...
  const didExecuteOrQueue = await executor.tryOrQueue(
    async () => {
      const tx = await addressesProvider.setPoolImpl(
        poolImplementationDeployment.address,
      );
      await tx.wait();
    },
    () => ({
      ...
      value: "0",
      data: addressesProvider.interface.encodeFunctionData("setPoolImpl", [
        poolImplementationDeployment.address,
      ]),
    }),
  );
  ...
  const flushed = await executor.flush(
    "Fraxtal lending security upgrade: set patched L2Pool implementation",
  );
