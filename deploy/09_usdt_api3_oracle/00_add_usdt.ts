...
import { API3_ORACLE_WRAPPER_ID, ORACLE_AGGREGATOR_ID } from "../../utils/oracle/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const usdtOracleAddress = config.oracleAggregator.api3OracleAssets.plainApi3OracleWrappers[usdtAddress];
  ...
  const { address: api3WrapperAddress } = await hre.deployments.get(API3_ORACLE_WRAPPER_ID);

  // Get the API3Wrapper contract instance
  const api3Wrapper = await hre.ethers.getContractAt("API3Wrapper", api3WrapperAddress, await hre.ethers.getSigner(dusdDeployer));

  console.log(`Setting proxy for USDT...`);
  await api3Wrapper.setProxy(usdtAddress, usdtOracleAddress);
  console.log(`Proxy set for USDT: Asset ${usdtAddress}, Oracle ${usdtOracleAddress}`);
  ...
  console.log(`OracleAggregator set for USDT: Asset ${usdtAddress}, Oracle ${api3WrapperAddress}`);
...
func.tags = ["point-api3-oracle-wrapper", "oracle-aggregator", "point-api3-usdt"];
