...
import { CURVE_API3_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID, ORACLE_AGGREGATOR_ID } from "../../utils/oracle/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const oracleAggregatorContract = await hre.ethers.getContractAt("OracleAggregator", oracleAggregatorAddress, deployer);
  ...
  const thresholdFeeds = Object.fromEntries(
    Object.entries(config.oracleAggregator.api3OracleAssets.api3OracleWrappersWithThresholding).filter(([key]) =>
      thresholdAddresses.includes(key),
    ),
  );
...
func.tags = ["oracle-aggregator", "point-api3-oracle-wrapper", "point-curve-oracle-wrapper"];
