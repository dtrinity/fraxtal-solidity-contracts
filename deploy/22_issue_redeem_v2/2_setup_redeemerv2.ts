...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const executor = new GovernanceExecutor(hre, await hre.ethers.getSigner(dusdDeployer), config.safeConfig);
  await executor.initialize();

  console.log(`\n=== Deploy RedeemerV2 for dUSD ===`);

  const { address: oracle } = await deployments.get(ORACLE_AGGREGATOR_ID);
  const { address: vault } = await deployments.get(COLLATERAL_VAULT_CONTRACT_ID);

  const tokenAddress = config.dusd.address;
  const initialFeeReceiver = config.dStables?.dUSD?.initialFeeReceiver || dusdDeployer;
  ...
  try {
    ...
    if (fees && Object.keys(fees).length > 0) {
      const redeemer = await hre.ethers.getContractAt("RedeemerV2", result.address, await hre.ethers.getSigner(dusdDeployer));

      for (const [asset, feeBps] of Object.entries(fees)) {
        try {
          const overridden = await (redeemer as any).isCollateralFeeOverridden(asset);
          const current = await (redeemer as any).collateralRedemptionFeeBps(asset);
          ...
          if (!overridden || currentBps !== desired) {
            ...
            console.log(`    ⚙️ Set fee override for ${asset} to ${feeBps as number}`);
          } else {
            console.log(`    ✓ Fee override for ${asset} already ${currentBps}`);
          }
        } catch (e) {
          console.log(`    ⚠️ Could not set fee for ${asset}: ${(e as Error).message}`);
        }
      }
    } else {
      console.log("    ℹ️ No per-collateral fee overrides specified in config.");
    }
  } catch (e) {
    console.log(`    ⚠️ Per-collateral fee setup skipped: ${(e as Error).message}`);
  }
  ...
  try {
    const vaultContract = await hre.ethers.getContractAt("CollateralHolderVault", vault, await hre.ethers.getSigner(dusdDeployer));
    ...
    const legacyRedeemerIds = [REDEEMER_CONTRACT_ID, dUSD_REDEEMER_WITH_FEES_CONTRACT_ID];
