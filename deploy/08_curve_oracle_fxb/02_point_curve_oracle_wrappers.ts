...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const { address: curveWrapperAddress } = await hre.deployments.get(CURVE_API3_COMPOSITE_WRAPPER_WITH_THRESHOLDING_ID);
  ...
  for (const [assetAddress, _feedConfig] of Object.entries(curveFeeds)) {
    ...
    if (testPrice == 0n) {
      throw new Error(`The Curve oracle wrapper has not been set for ${assetAddress}`);
    }

    console.log(`Setting Curve oracle wrapper for ${assetAddress} to`, curveWrapperAddress);
