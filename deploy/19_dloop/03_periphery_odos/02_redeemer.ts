...
import { DLOOP_REDEEMER_ODOS_ID } from "../../../utils/vault/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (isLocalNetwork(hre.network.name)) {
    console.log("Skipping DLoopRedeemerOdos deployment on local network");
    return false;
  }
  ...
  if (!config.dLoop) {
    console.log(`No dLOOP configuration defined for network ${hre.network.name}. Skipping Odos redeemer deployment.`);
    return false;
  }

  // Skip if no redeemers section or Odos redeemer is defined
  if (!config.dLoop.redeemers || !config.dLoop.redeemers.odos) {
    console.log(`Odos redeemer not defined for network ${hre.network.name}. Skipping.`);
    return false;
  }

  const odosConfig = config.dLoop.redeemers.odos;
  ...
  console.log("Deploying DLoopRedeemerOdos...");
  ...
  console.log("DLoopRedeemerOdos deployed successfully");
  return true;
};

func.id = DLOOP_REDEEMER_ODOS_ID;
