...
import { DLOOP_INCREASE_LEVERAGE_ODOS_ID } from "../../../utils/vault/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (isLocalNetwork(hre.network.name)) {
    console.log("Skipping DLoopIncreaseLeverageOdos deployment on local network");
    return false;
  }
  ...
  if (!config.dLoop) {
    console.log(`No dLOOP configuration defined for network ${hre.network.name}. Skipping Odos increase leverage deployment.`);
    return false;
  }

  // Skip if no increase leverage section or Odos increase leverage is defined
  if (!config.dLoop.increaseLeverage || !config.dLoop.increaseLeverage.odos) {
    console.log(`Odos increase leverage not defined for network ${hre.network.name}. Skipping.`);
    return false;
  }

  const odosConfig = config.dLoop.increaseLeverage.odos;
  ...
  console.log("Deploying DLoopIncreaseLeverageOdos...");
  ...
  // Deploy DLoopIncreaseLeverageOdos
  ...
  console.log("DLoopIncreaseLeverageOdos deployed successfully");
  return true;
};

func.id = DLOOP_INCREASE_LEVERAGE_ODOS_ID;
