...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (isLocalNetwork(hre.network.name)) {
    console.log("Skipping DLoopDepositorUniswapV3 deployment on local network");
    return false;
  }
  ...
  console.log("Deploying DLoopDepositorUniswapV3...");
  ...
  // Deploy DLoopDepositorUniswapV3
  await deployContract(
    hre,
    DLOOP_DEPOSITOR_UNISWAP_V3_ID,
    [
    ...
    "DLoopDepositorUniswapV3",
  );

  console.log("DLoopDepositorUniswapV3 deployed successfully");
  return true;
};

func.id = DLOOP_DEPOSITOR_UNISWAP_V3_ID;
func.tags = ["dloop", "periphery", "uniswap-v3", "depositor"];
