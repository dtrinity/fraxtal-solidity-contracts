...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (!incentivesProxyDeployment) {
    console.log(
      "IncentivesProxy not deployed. Skipping dLend rewards manager deployment.",
    );
    return;
  }
  ...
  const emissionManagerDeployment =
    await deployments.getOrNull(EMISSION_MANAGER_ID);

  if (!emissionManagerDeployment) {
    console.log(
      "EmissionManager not deployed. Skipping dLend rewards manager deployment.",
    );
    return;
  }

  // Get pool data provider to fetch aToken addresses
  const poolDataProviderDeployment = await deployments.getOrNull(
    POOL_DATA_PROVIDER_ID,
  );

  if (!poolDataProviderDeployment) {
    console.log(
      "PoolDataProvider not deployed. Skipping dLend rewards manager deployment.",
    );
    return;
  }
  ...
  for (const instanceKey in config.dStake) {
    ...
    const wrappedATokenDeploymentId =
      instanceKey === "sdUSD"
        ? dUSD_A_TOKEN_WRAPPER_ID
        : `${instanceKey}_ATokenWrapper`;
    const wrappedATokenDeployment = await deployments.getOrNull(
      wrappedATokenDeploymentId,
    );

    if (!wrappedATokenDeployment) {
      console.log(
        `Wrapped aToken not deployed for ${instanceKey}. Skipping reward manager deployment.`,
      );
      continue;
    }
    ...
    if (aTokenAddress === ethers.ZeroAddress) {
      console.log(
        `No aToken found for underlying stable ${underlyingStablecoinAddress}. Skipping reward manager for ${instanceKey}.`,
      );
      continue;
    }
    ...
    if (deployment.newlyDeployed) {
      console.log(
        `Deployed DStakeRewardManagerDLend for ${instanceKey} at ${deployment.address}`,
      );
      ...
      if (!deployerIsAdmin) {
        ...
        if (
          targetRewardsManager !== deployer &&
          (await rewardManager.hasRole(REWARDS_MANAGER_ROLE, deployer))
        ) {
        ...
        if (
          ...
          (await rewardManager.hasRole(DEFAULT_ADMIN_ROLE, deployer))
        ) {
      ...
      console.log(
        `DStakeRewardManagerDLend for ${instanceKey} already deployed at ${deployment.address}`,
      );
    }
  }
