...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  if (!isLocalNetwork(hre.network.name) && !isTestnetNetwork(hre.network.name)) {
    console.log(`Skipping ${MARKET_NAME} deploying MockFlashLoanReceiver on ${hre.network.name}`);
    return false;
  }

  const { lendingDeployer } = await hre.getNamedAccounts();

  return deployMockFlashLoanReceiver(hre, await hre.ethers.getSigner(lendingDeployer));
};
...
func.dependencies = ["before-deploy", "lbp-core", "lbp-periphery-pre", "lbp-provider", "lbp-init-pool", "lbp-oracles"];
