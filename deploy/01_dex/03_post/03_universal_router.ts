...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (!config.dex) {
    console.log(
      "Skipping Universal Router deployment - dex config not populated",
    );
    return false;
  }
  ...
  const { address: dexFactoryAddress } = await hre.deployments.get(
    UNISWAP_V3_FACTORY_ID,
  );

  const poolContractArtifact =
    await hre.deployments.getArtifact("UniswapV3Pool");
  const poolInitCodeHash = keccak256(
    ["bytes"],
    [`${poolContractArtifact.bytecode}`],
  );
