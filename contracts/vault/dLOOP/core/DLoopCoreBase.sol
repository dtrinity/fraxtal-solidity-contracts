...
abstract contract DLoopCoreBase is ERC4626, Ownable {
    ...
    error DepositInsufficientToSupply(uint256 currentBalance, uint256 newTotalAssets);
    error UnexpectedLossOfPrincipal(uint256 principalBefore, uint256 principalAfter);
    error CollateralLessThanDebt(uint256 totalCollateralBase, uint256 totalDebtBase);
    error InsufficientShareBalanceToRedeem(address owner, uint256 sharesToRedeem, uint256 shareBalance);
    error InvalidMaxWithdrawAfterRepay(
        address token,
        uint256 maxWithdrawUnderlyingBeforeRepay,
        uint256 maxWithdrawUnderlyingAfterRepay
    );
    error WithdrawableIsLessThanRequired(address token, uint256 assetToRemoveFromLending, uint256 withdrawableAmount);
    ...
    error UnexpectedLossOfWithdrawableAmount(uint256 withdrawableAmount, uint256 receivedAmount);
     ...
     */
    function getAssetPriceFromOracle(address asset) public view virtual returns (uint256);
     ...
     */
    function _supplyToPool(address token, uint256 amount, address onBehalfOf) internal virtual;
     ...
     */
    function _borrowFromPool(address token, uint256 amount, address onBehalfOf) internal virtual;
     ...
     */
    function _repayDebt(address token, uint256 amount, address onBehalfOf) internal virtual;
     ...
     */
    function _withdrawFromPool(address token, uint256 amount, address onBehalfOf) internal virtual;
     ...
     */
    function _getBaseAssetAddressAndSymbol() internal view virtual returns (address, string memory);
    ...
    function _getTotalCollateralAndDebtOfUserInBase(
        address user
    ) internal view virtual returns (uint256 totalCollateralBase, uint256 totalDebtBase);
     ...
     */
    function _getMaxWithdrawAmount(address user, address asset) internal view returns (uint256) {
    ...
    function getLeveragedAssets(uint256 assets) public view returns (uint256) {
        return (assets * TARGET_LEVERAGE_BPS) / Constants.ONE_HUNDRED_PERCENT_BPS;
    }
    ...
    function _deposit(address caller, address receiver, uint256 assets, uint256 shares) internal override {
        ...
        underlyingAsset.safeTransferFrom(caller, address(this), leveragedAssets);
    ...
    function _depositToPoolImplementation(
        ...
        uint256 currentUnderlyingAssetBalance = underlyingAsset.balanceOf(address(this));
        if (currentUnderlyingAssetBalance < newTotalAssets) {
            revert DepositInsufficientToSupply(currentUnderlyingAssetBalance, newTotalAssets);
        }

        uint256 maxDStableToBorrowBeforeSupply = _getMaxWithdrawAmount(address(this), address(dStable));

        // Supply the underlying asset to the lending pool
        _supplyToPool(address(underlyingAsset), newTotalAssets, address(this));

        uint256 maxDStableToBorrowAfterSupply = _getMaxWithdrawAmount(address(this), address(dStable));
        ...
        // Now, this value > 0
        uint256 maxDStableToBorrow = maxDStableToBorrowAfterSupply - maxDStableToBorrowBeforeSupply;
        ...
        if (dStableBalanceAfterBorrow - dStableBalanceBeforeBorrow < maxDStableToBorrow) {
    ...
    function _withdraw(
        ...
        uint256 assetsToRemoveFromLending = (principalAssetsToRemove * TARGET_LEVERAGE_BPS) /
        ...
        uint256 dStableToRepay = ((assetsToRemoveFromLending *
            (getAssetPriceFromOracle(address(underlyingAsset)) * 10 ** dStable.decimals())) /
        ...
        _withdrawFromPool(address(underlyingAsset), withdrawableUnderlyingAmount, address(this));
        ...
        if (underlyingAssetBalanceAfter < underlyingAssetBalanceBefore) {
            revert UnexpectedLossOfPrincipal(underlyingAssetBalanceBefore, underlyingAssetBalanceAfter);
        }

        uint256 receivedUnderlyingAmount = underlyingAssetBalanceAfter - underlyingAssetBalanceBefore;
        ...
        if (receivedUnderlyingAmount < withdrawableUnderlyingAmount) {
            revert UnexpectedLossOfWithdrawableAmount(withdrawableUnderlyingAmount, receivedUnderlyingAmount);
        }

        // Transfer the remaining assets to the receiver
        underlyingAsset.safeTransfer(receiver, receivedUnderlyingAmount);

        emit Withdraw(caller, receiver, owner, receivedUnderlyingAmount, shares);
    }
    ...
    function _getWithdrawAmountThatKeepCurrentLeverage(
        ...
        //    <=> x = y * T/(T-1)
        //
        uint256 difference = maxWithdrawAmountAfterRepay - maxWithdrawAmountBeforeRepay;
        ...
        return (difference * currentLeverageBps) / (currentLeverageBps - Constants.ONE_HUNDRED_PERCENT_BPS);
    }

    /* Rebalance */

    function increaseLeverage(uint256 assetAmount, uint256 minPriceInBase) public {
        ...
        uint256 assetAmountInBase = (assetAmount * assetPriceInBase) / (10 ** underlyingAsset.decimals());

        (uint256 totalCollateralBase, uint256 totalDebtBase) = _getTotalCollateralAndDebtOfUserInBase(address(this));

        uint256 currentSubsidyBps = _getCurrentSubsidyBps();

        uint256 dStablePriceInBase = getAssetPriceFromOracle(address(dStable));
        uint256 borrowedDStableInBase = (assetAmountInBase * (Constants.ONE_HUNDRED_PERCENT_BPS + currentSubsidyBps)) /
            Constants.ONE_HUNDRED_PERCENT_BPS;

        uint256 newLeverageBps = ((totalCollateralBase + assetAmountInBase) * Constants.ONE_HUNDRED_PERCENT_BPS) /
            (totalCollateralBase + assetAmountInBase - totalDebtBase - borrowedDStableInBase);

        uint256 currentLeverageBps = getCurrentLeverageBps();

        if (newLeverageBps > TARGET_LEVERAGE_BPS || newLeverageBps <= currentLeverageBps) {
            revert IncreaseLeverageOutOfRange(newLeverageBps, TARGET_LEVERAGE_BPS, currentLeverageBps);
        }
        ...
        // Borrow more dStable
        uint256 borrowedDStable = (borrowedDStableInBase * (10 ** dStable.decimals())) / dStablePriceInBase;
    ...
    function decreaseLeverage(uint256 dStableAmount, uint256 maxPriceInBase) public {
        uint256 assetPriceInBase = getAssetPriceFromOracle(address(underlyingAsset));
        if (assetPriceInBase > maxPriceInBase) {
            revert ExceedMaxPrice(assetPriceInBase, maxPriceInBase);
        }

        uint256 dStableAmountInBase = (dStableAmount * getAssetPriceFromOracle(address(dStable))) /
            (10 ** dStable.decimals());

        (uint256 totalCollateralBase, uint256 totalDebtBase) = _getTotalCollateralAndDebtOfUserInBase(address(this));

        uint256 currentSubsidyBps = _getCurrentSubsidyBps();
        uint256 withdrawnAssetsBase = (dStableAmountInBase * (Constants.ONE_HUNDRED_PERCENT_BPS + currentSubsidyBps)) /
            Constants.ONE_HUNDRED_PERCENT_BPS;

        uint256 newLeverageBps = ((totalCollateralBase - withdrawnAssetsBase) * Constants.ONE_HUNDRED_PERCENT_BPS) /
            (totalCollateralBase - withdrawnAssetsBase - totalDebtBase + dStableAmountInBase);

        uint256 currentLeverageBps = getCurrentLeverageBps();
        if (newLeverageBps < TARGET_LEVERAGE_BPS || newLeverageBps >= currentLeverageBps) {
            revert DecreaseLeverageOutOfRange(newLeverageBps, TARGET_LEVERAGE_BPS, currentLeverageBps);
        }
        ...
        // Withdraw collateral
        uint256 withdrawnAssets = (withdrawnAssetsBase * (10 ** underlyingAsset.decimals())) / assetPriceInBase;

        uint256 underlyingAssetBalanceBefore = underlyingAsset.balanceOf(address(this));

        _withdrawFromPool(address(underlyingAsset), withdrawnAssets, address(this));
        ...
        if (receivedUnderlyingAmount < withdrawnAssets) {
            revert UnexpectedLossOfWithdrawableAmount(withdrawnAssets, receivedUnderlyingAmount);
        }
    ...
    function _getCurrentSubsidyBps() internal view returns (uint256) {
        ...
        if (currentLeverageBps > TARGET_LEVERAGE_BPS) {
            subsidyBps =
                ((currentLeverageBps - TARGET_LEVERAGE_BPS) * Constants.ONE_HUNDRED_PERCENT_BPS) /
                TARGET_LEVERAGE_BPS;
        } else {
            subsidyBps =
                ((TARGET_LEVERAGE_BPS - currentLeverageBps) * Constants.ONE_HUNDRED_PERCENT_BPS) /
                TARGET_LEVERAGE_BPS;
        }
    ...
    function getCurrentLeverageBps() public view returns (uint256) {
        ...
        return ((totalCollateralBase * Constants.ONE_HUNDRED_PERCENT_BPS) / (totalCollateralBase - totalDebtBase));
    }
    ...
    function setLeverageBounds(
        ...
        if (
            _lowerBoundTargetLeverageBps >= TARGET_LEVERAGE_BPS || TARGET_LEVERAGE_BPS >= _upperBoundTargetLeverageBps
        ) {
            revert InvalidLeverageBounds(
                _lowerBoundTargetLeverageBps,
                TARGET_LEVERAGE_BPS,
