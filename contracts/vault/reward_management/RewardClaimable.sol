...
abstract contract RewardClaimable is AccessControl, ReentrancyGuard {
    ...
    error ExchangeAssetBalanceIncreased(uint256 exchangeAssetBalanceBefore, uint256 exchangeAssetBalanceAfter);
    error DepositAmountMismatch(uint256 actualDepositAmount, uint256 expectedDepositAmount);
    ...
    constructor(
        ...
        if (_maxTreasuryFeeBps > Constants.ONE_HUNDRED_PERCENT_BPS) {
    ...
    function getTreasuryFee(uint256 amount) public view returns (uint256) {
        return (amount * treasuryFeeBps) / Constants.ONE_HUNDRED_PERCENT_BPS;
    }
     ...
     */
    function compoundRewards(uint256 amount, address[] calldata rewardTokens, address receiver) public nonReentrant {
