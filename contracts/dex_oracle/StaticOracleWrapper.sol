...
contract StaticOracleWrapper is IPriceOracleGetter {
    ...
    constructor(
        ...
        (bool success, bytes memory data) = _quoteToken.staticcall(abi.encodeWithSignature("decimals()"));
    ...
    function getAssetPrice(address _baseToken) external view returns (uint256) {
        ...
        (_baseAmount, queriedPools) = STATIC_ORACLE.quoteAllAvailablePoolsWithTimePeriod(
            QUOTE_TOKEN_AMOUNT,
            QUOTE_TOKEN,
            _baseToken,
        ...
        (bool success, bytes memory data) = _baseToken.staticcall(abi.encodeWithSignature("decimals()"));
        ...
        uint256 _price = (QUOTE_TOKEN_AMOUNT * _baseTokenUnit * PRICE_UNIT) / (_baseAmount * QUOTE_TOKEN_UNIT);
