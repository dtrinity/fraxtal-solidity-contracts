...
async function main(): Promise<void> {
  ...
  const underlyingTokenAddress = config.dLoopCurve?.vaults[0]
    .underlyingAssetAddress as string;

  const { tokenInfo: underlyingTokenInfo, contract: underlyingTokenContract } =
    await getTokenContractForAddress(dexDeployer, underlyingTokenAddress);
  ...
  const lowerBoundTargetLeverageBps =
    await dLOOPCurve.LOWER_BOUND_TARGET_LEVERAGE_BPS();
  const upperBoundTargetLeverageBps =
    await dLOOPCurve.UPPER_BOUND_TARGET_LEVERAGE_BPS();
  console.log(
  ...
  let res = await dLOOPCurve.withdrawDebug(
  ...
  console.log(
    "Share burned: ",
    (shareBalanceAfter - shareBalanceBefore).toString(),
  );
  ...
  // Check current leverage bps
  console.log(
    "Current leverage bps: ",
    (await dLOOPCurve.getCurrentLeverageBps()).toString(),
  );
  console.log("");
}
