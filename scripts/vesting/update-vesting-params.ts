...
async function main() {
  ...
  const vestingContract = await ethers.getContractAt("ERC20VestingNFT", vestingDeployment.address, deployer);

  console.log("Updating vesting contract parameters...");

  console.log(`Updating maxTotalSupply to ${vestingConfig.maxTotalSupply}...`);
  const tx1 = await vestingContract.setMaxTotalSupply(vestingConfig.maxTotalSupply);
  console.log(`Transaction hash: ${tx1.hash}`);
  await tx1.wait();
  console.log("setMaxTotalSupply successful.");

  console.log(`Updating minDepositAmount to ${vestingConfig.minDepositThreshold}...`);
  const tx2 = await vestingContract.setMinDepositAmount(vestingConfig.minDepositThreshold);
