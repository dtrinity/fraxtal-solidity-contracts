...
async function main() {
  ...
  const { lendingDeployer, dusdDeployer } = await getNamedAccounts();
  const deployer = lendingDeployer || dusdDeployer;
  ...
  for (const c of scan.rolesContracts) {
    ...
    for (const role of nonAdminRolesHeldByDeployer) {
      try {
        const governanceHasRole: boolean = await (contract as any).hasRole(
          role.hash,
          governance,
        );
        ...
        const deployerHasRole: boolean = await (contract as any).hasRole(
          role.hash,
          deployer,
        );
        if (deployerHasRole) {
    ...
    if (adminRole) {
      try {
        const governanceHasAdmin: boolean = await (contract as any).hasRole(
          adminRole.hash,
          governance,
        );
        if (!governanceHasAdmin) {
          ...
          const tx = await (contract as any).grantRole(
            adminRole.hash,
            governance,
          );
          await tx.wait();
        ...
        const deployerHasAdmin: boolean = await (contract as any).hasRole(
          adminRole.hash,
          deployer,
        );
        if (deployerHasAdmin) {
          ...
          const tx2 = await (contract as any).renounceRole(
            adminRole.hash,
            deployer,
          );
          await tx2.wait();
